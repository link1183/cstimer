...
(function () {
  ...
  var edgeY = [
    -Math.sin(PI * 0.1) + (cfrac - 1) / 2,
    -(1 + cfrac) / 2,
    -cfrac,
    -Math.cos(PI * 0.4) * cfrac,
  ];
  var centX = [
    Math.sin(PI * 0.0) * cfrac,
    Math.sin(PI * 0.4) * cfrac,
    Math.sin(PI * 0.8) * cfrac,
    Math.sin(PI * 1.2) * cfrac,
    Math.sin(PI * 1.6) * cfrac,
  ];
  var centY = [
    -Math.cos(PI * 0.0) * cfrac,
    -Math.cos(PI * 0.4) * cfrac,
    -Math.cos(PI * 0.8) * cfrac,
    -Math.cos(PI * 1.2) * cfrac,
    -Math.cos(PI * 1.6) * cfrac,
  ];

  function faceTrans(width, theta) {
    var hc = Math.tan(Math.PI * 0.3) / Math.tan(Math.PI * 0.4);
    return [
      width * Math.cos(theta + 0.5 * PI),
      -width * Math.sin(theta + 0.5 * PI) * hc,
      width * (off1X + ((Math.cos(theta) * efrac2) / 2) * (1 + hc)),
      width * Math.sin(theta + 0.5 * PI),
      width * hc * Math.cos(theta + 0.5 * PI),
      width * (off1Y + ((Math.sin(theta) * efrac2) / 2) * (1 + hc)),
    ];
  }
  ...
  function renderQMinx(twisty) {
    ...
    drawFace(
      state,
      22,
      [width, off1X + 0 * efrac2, off1Y + 0 * efrac2],
      PI * 0.6,
      ctx,
    );
  ...
  function drawFace(state, baseIdx, trans, rot, ctx) {
    for (var i = 0; i < 5; i++) {
      drawPolygon(
        ctx,
        colors[state[baseIdx + i]],
        Rotate([cornX, cornY], ((PI * 2) / 5) * i + rot),
        trans,
      );
      drawPolygon(
        ctx,
        colors[state[baseIdx + i + 5]],
        Rotate([edgeX, edgeY], ((PI * 2) / 5) * i + rot),
        trans,
      );
    }
    drawPolygon(
      ctx,
      colors[state[baseIdx + 10]],
      Rotate([centX, centY], rot),
      trans,
    );
  }
  ...
  function initQMinx(scene, options) {
    ...
    this.parseScramble = function (scramble, addPreScr) {
      if (!scramble || /^\s*$/.exec(scramble)) {
        ...
        scramble.replace(
          /(?:^|\s*)(?:([DLR])(\+\+?|--?)|(U|F|D?B?R|D?B?L|D|B)(\d?)('?)|\[([ufrl])('?)\])(?:$|\s*)/g,
    ...
    this.move2str = function (move) {
      ...
      if (nlayer == 6) {
        ...
        return (
          ["U", "F", "R", "BR", "BL", "L", "D", "B", "DBL", "DL", "DR", "DBR"][
            axis
          ] + (pow > 0 ? "" : "'")
        );
      }
