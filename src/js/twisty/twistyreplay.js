var replay = execMain(function () {
  ...
  function procClick(e) {
    ...
    if (key == "l") {
      ...
      if (
        _status == 0 &&
        playMoves.length > 0 &&
        curTime >= normTime(playMoves.at(-1)[1])
      ) {
        goToStep(0);
      }
      setStatus(1 - _status);
  ...
  function execLoop() {
    ...
    while (
      curPlayIdx < playMoves.length &&
      normTime(playMoves[curPlayIdx][1]) <= ts
    ) {
  ...
  function updateTime() {
    ...
    txtTime.html(
      (curTime >= 0 ? kernel.pretty(normTime(curTime, true)) : "--") +
        "/" +
        kernel.pretty(lastTs),
    );
    updateMoveTd();
  ...
  function parseRecons(recons) {
    ...
    algSpan.attr(
      ...
      "https://alg.cubing.net/?alg=" +
        ...
        "&setup=" +
        encodeURIComponent(curScramble || ""),
    );
    ...
    for (var i = 0; i < playMoves.length; i++) {
      maxMoveLen = Math.max(
        maxMoveLen,
        ("" + puzzleObj.move2str(playMoves[i][0])).length,
      );
    }
    curPlayIdx = 0;
    curTime = normTime(-1);
    ...
    shareURL = new URL(
      "?vrcreplay=" +
        LZString.compressToEncodedURIComponent(
          JSON.stringify([scramble, recons, puzzle]),
        ),
      location,
    ).toString();
    var options = {
  ...
  function bindDomElem(elem, scramble, recons, puzzle) {
    ...
    elem
      .unbind("click")
      .click(popupReplay.bind(null, scramble, recons, puzzle));
  }

  $(function () {
    div = $('<table style="height:98%">');
    puzzleDiv = $('<td style="height:80%">');
    var spanTpl = $.format.bind(
      null,
    ...
    playSpan = $(spanTpl(["p", "\ue800"]));
    moveSeqTd = $(
      '<td style="width:0%;font-family:monospace;white-space:pre;">',
    );
    div.append(
      $("<tr>").append(
        $("<td>").append(
    ...
    if (req) {
      ...
      try {
        req = JSON.parse(
          LZString.decompressFromEncodedURIComponent(decodeURIComponent(req)),
        );
        setTimeout(function () {
