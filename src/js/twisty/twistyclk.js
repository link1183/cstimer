(function () {
  ...
  var sidesUV = [axify(xx, zyi, yz), axify(xxi, zyi, yzi)];
    ...
    for (var i = 0; i < numSides; i++) {
      for (var xy = 0; xy < 9; xy++) {
        ...
        for (var theta = 0; theta < 12; theta++) {
          ...
          var transformationMatrix = sidesUV[i]
            .clone()
            .multiplySelf(
        ...
        for (var tidx = 0; tidx < 4; tidx++) {
          ...
          for (var k = 0; k < point[tidx].length; k++) {
            curf[k] = [
              point[tidx][k][0] * 0.2 * amp,
              point[tidx][k][1] * 0.2 * amp,
            ];
          }
      ...
      for (var xy = 0; xy < 4; xy++) {
        ...
        for (var tidx = 0; tidx < 4; tidx++) {
          ...
          for (var k = 0; k < button[tidx].length; k++) {
            curf[k] = [
              button[tidx][k][0] * 0.2 * amp,
              button[tidx][k][1] * 0.2 * amp,
            ];
          }
          var sticker = new THREE.Object3D();
          var stickerInterior = new THREE.Mesh(new THREE.Ploy(curf), meshes);
          ...
          var transformationMatrix = sidesUV[i]
            ...
            .multiplySelf(
              new THREE.Matrix4().setRotationAxis(
                zz,
                (Math.TAU / 3) * tidx + (Math.TAU / 2) * i,
              ),
            );
          sticker.matrix.copy(transformationMatrix);
          sticker.matrixAutoUpdate = false;
    ...
    function animateMoveCallback(twisty, currentMove, moveProgress, moveStep) {
      ...
      if (axis == mY) {
        ...
        for (var i = 0; i < 18; i++) {
          ...
          for (var j = 0; j < state[i].length; j++) {
            var sticker = state[i][j];
            sticker[1].matrix.multiply(
              sticker[1].matrix,
              (i < 9) ^ twisty.internalState[22] ? rots : rotsi,
            );
            sticker[1].update();
          }
        }
      ...
      if (axis == mY) {
        ...
        for (var i = 0; i < 2; i++) {
          ...
          for (var j = 0; j < state[idx].length; j++) {
            ...
            sticker[1].update();
            var color =
              cubeOptions.faceColors[
                3 + (twisty.internalState[18 + axis - mbUR] ^ i)
              ];
