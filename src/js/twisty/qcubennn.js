...
(function () {
  ...
  function renderTopCube(isTwoLook, twisty) {
    ...
    var width = twisty.twistyScene.fixSize(
      size + 1 + gap * 3 + offset,
      size * 2 + gap * 3 - 1 + offset,
    );
    ...
    for (var i = 0; i < size; i++) {
      ...
      // R
      drawPolygon(ctx, colors[posit[(4 * size + 0) * size + ii]], piece, [
        width,
        size + gap * 2 + offset,
        gap + offset,
      ]);
      // B and L
      if (!isTwoLook) {
        drawPolygon(ctx, colors[posit[(1 * size + 0) * size + ii]], piece, [
          width,
          gap + 0,
          gap * 2 + 1,
        ]);
        ...
        drawPolygon(ctx, colors[posit[(2 * size + 0) * size + i]], piece, [
          width,
          1 + gap * 2,
          gap,
        ]);
      ...
      for (var j = 0; j < size; j++) {
        ...
        drawPolygon(ctx, colors[posit[(3 * size + j) * size + i]], piece, [
          width,
          gap + offset,
          gap + offset,
        ]);
        if (j < 2) {
          drawPolygon(ctx, colors[posit[(5 * size + j) * size + i]], piece, [
            width,
            gap + offset,
            size + gap * 2 + offset,
          ]);
    ...
    if (size > 5) {
      drawPolygon(
        ...
        [width, size * 0.5 + 1 + gap * 2, size * 0.5 + gap * 2 + 1],
      );
    }
  }
  ...
  function renderQCube(twisty) {
    ...
    for (var i = 0; i < size; i++) {
      ...
      // L and R
      if (i != 0) {
        drawPolygon(ctx, colors[posit[(1 * size + i) * size + 0]], piece, [
          width,
          gap,
          size + gap * 2,
        ]);
        drawPolygon(ctx, colors[posit[(4 * size + i) * size + 0]], piece, [
          width,
          size + 1 + gap * 3,
          size + gap * 2,
        ]);
    ...
    drawPolygon(ctx, colors[posit[(1 * size + 0) * size + 0]], piece2, [
      width,
      gap,
      gap + size - 1,
    ]);
    drawPolygon(ctx, colors[posit[(4 * size + 0) * size + 0]], piece2, [
      width,
      size + 1 + gap * 3,
      gap + size - 1,
    ]);
    if (size > 5) {
      drawPolygon(
        ...
        [width, size * 0.5 + 1 + gap * 2, size * 0.5 + gap],
      );
      drawPolygon(
        ctx,
        "#fff",
        ...
        [width, size * 0.5 + 1 + gap * 2, size * 1.5 + gap * 2],
      );
    }
  }
        ...
        for (var f = 0; f < 6; f++) {
          for (var j = 0; j < size; j++) {
            ...
            for (var i = 0; i < size; i++) {
              ...
              facelet[((f + 3) % 6) * s2 + j * size + i] = "DLBURF".charAt(
                this.posit[f * s2 + y * size + x],
              );
            }
          }
        }
        return cubeutil.getProgress(
          facelet.join(""),
          kernel.getProp("vrcMP", "n"),
        );
      }
      var cnt = 0;
      for (var i = 0; i < 6; i++) {
