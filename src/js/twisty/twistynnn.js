  ...
  function createCubeTwisty(twistyScene, twistyParameters) {
    ...
    for (var i = 0; i < numSides; i++) {
      ...
      for (var su = 0; su < cubeOptions.dimension; su++) {
        for (var sv = 0; sv < cubeOptions.dimension; sv++) {
          ...
          var stickerInterior = new THREE.Mesh(
            new THREE.Plane(cubeOptions.stickerWidth, cubeOptions.stickerWidth),
            meshes,
          );
          stickerInterior.doubleSided = cubeOptions.doubleSided;
          sticker.addChild(stickerInterior);

          var transformationMatrix = new THREE.Matrix4().multiply(
            sidesUV[i],
      ...
      for (var i = 0; i < handMarks.length; i++) {
        ...
        sticker.matrix.copy(
          new THREE.Matrix4().multiply(
            ...
            new THREE.Matrix4().setTranslation(
              su * (dimension - 2 * offset),
              dimension * (hsq2 * 2 + 0.05),
              0,
            ),
          ),
        );
    ...
    function animateMoveCallback(twisty, currentMove, moveProgress, moveStep) {
      ...
      rots.setRotationAxis(
        sidesRotAxis[currentMove[2]],
        (moveStep * currentMove[3] * Math.TAU) / 4,
      );
    ...
    function isSolved(twisty) {
      ...
      if (twisty.options.dimension == 3) {
        var curProgress = cubeutil.getProgress(
          getFacelet(twisty),
          kernel.getProp("vrcMP", "n"),
        );
        return twistyScene.isMoveFinished()
          ? curProgress
          : Math.max(1, curProgress);
      }
      ...
      for (var faceIndex = 0; faceIndex < numSides - 1; faceIndex++) {
        ...
        for (
          var stickerIndex = 1, faceStickerslength = faceStickers.length;
          stickerIndex < faceStickerslength;
          stickerIndex++
        ) {
          var sticker = faceStickers[stickerIndex];
          if (
            0.5 < Math.abs(matrixVector3Dot(sticker[0], normVector) - dimension)
          ) {
            return 1;
          }
        }
