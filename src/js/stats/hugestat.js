...
var crossSessionStats = execMain(
  function (kpretty, round, kpround) {
    ...
    function hugeTimeAt(idx) {
      return hugeTimes[idx][0][0] == -1
        ? -1
        : ~~((hugeTimes[idx][0][0] + hugeTimes[idx][0][1]) / roundMilli) *
            roundMilli;
    }
    ...
    function updateInfo() {
      ...
      var dateThreshold =
        dateSelect.val() == -1
          ? -1
          : ~~(+new Date() / 1000) - dateSelect.val() * 86400;
      for (var i = 0; i < sessionN; i++) {
        ...
        if (
          selectedScr != "*" &&
          (sessionData[idx]["opt"]["scrType"] || "333") != selectedScr
        ) {
          continue;
        }
        loadproc = loadproc.then(
          function (idx) {
    ...
    function updateSpan() {
      ...
      s.push(
        '<span class="click" data="tt">' +
          hlstr[4].replace(
            "%d",
            hugeStats.timesLen - numdnf + "/" + hugeStats.timesLen,
          ) +
          ", " +
          hlstr[9].replace("%v", kpround(sessionmean)) +
          "</span>\n",
      );
      s.push(
        "<span>" + hlstr[12].replace("%d", kpretty(totalTime)) + "</span>\n",
      );
      s.push(
        hlstr[0] +
          ": " +
          '<span class="click" data="bs">' +
          kpretty(hugeStats.bestTime) +
          "</span>",
      );
      s.push(
        " | " +
      ...
      for (var j = 0; j < avgSizes.length; j++) {
        ...
        if (hugeStats.timesLen >= size) {
          ...
          s.push(
            "<tr><td>" + hlstr[7 - (avgSizes[j] >>> 31)].replace("%mk", size),
          );
          s.push(
            '<td><span class="click" data="c' +
              ...
              j +
              '">' +
              kpround(hugeStats.lastAvg[j][0]) +
              " (σ=" +
              stats.trim(hugeStats.lastAvg[j][1], 2) +
              ")</span></td>",
          );
          s.push(
            '<td><span class="click" data="b' +
              ...
              j +
              '">' +
              kpround(hugeStats.bestAvg(j, 0)) +
              " (σ=" +
              stats.trim(hugeStats.bestAvg(j, 1), 2) +
              ")</span></td></tr>",
          );
        }
      }
    ...
    function updateSelector() {
      ...
      $.each(curSessionData, function (idx, val) {
        if (
          ...
          (val["opt"] || {})["scrType"] !=
            (prevSessionData[idx]["opt"] || {})["scrType"]
        ) {
          isModified = true;
        }
      });
      ...
      nameSelect
        .empty()
        .append($("<option />").val("*").html(STATS_XSESSION_NAME));
      scrSelect
        .empty()
        .append($("<option />").val("*").html(STATS_XSESSION_SCR));
      $.each(curSessionData, function (idx, val) {
        ...
        if ($.inArray(curScr, scrList) == -1) {
          scrList.push(curScr);
          scrSelect.append(
            $("<option />").val(curScr).html(scramble.getTypeName(curScr)),
          );
        }
      });
    }
