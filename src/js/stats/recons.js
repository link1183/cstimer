var recons = execMain(function () {
  ...
  var reconsClick = $("<div>").append(
    '<a target="_blank" class="exturl click"></a>',
  );
  ...
  var tableTh = $("<tr>").append(
    $('<th style="padding:0;">').append(methodSelect),
    "<th>" +
      titleStr[0] +
      "</th><th>" +
      titleStr[1] +
  ...
  MoveCounterLFMC.prototype.push = function (move) {
    ...
    this.moveCnt +=
      (newPow == 0 ? 0 : axisCnt) - (this.lastPow == 0 ? 0 : axisCnt);
    this.lastPow = newPow;
  };
  ...
  function calcRecons(times, method) {
    ...
    for (var i = 0; i < solution.length; i++) {
      ...
      if (effMove != undefined) {
        ...
        var axis = ~~(effMove / 3);
        stepMoves.push([
          "URFDLB".charAt(axis % 6) + " 2'".charAt(effMove % 3),
          c.tstamp,
        ]);
        if (axis >= 6) {
          // slice move
          stepMoves.push([
            "DLBURF".charAt(axis % 6) + "'2 ".charAt(effMove % 3),
            c.tstamp,
          ]);
        }
      }
  ...
  function renderResult(stepData, tidx, isPercent, scramble, solve) {
    ...
    for (var i = 0; i < stepData.length; i++) {
      ...
      totMov += val[3];
      var isSuperStep =
        sDataIdx[i] == sDataIdx[i + 1] && sDataIdx[i] != sDataIdx[i - 1];
      if (isSuperStep) {
        curSIdx = sDataIdx[i];
        ...
        str.push(
          $.format(trTpl, [
            ...
            (sval[2] / maxSubt) * 100,
            isPercent
              ? Math.round((sval[1] / sumSubt) * 1000) / 10 + "%"
              : kernel.pretty(sval[1]),
            isPercent
              ? Math.round((sval[2] / sumSubt) * 1000) / 10 + "%"
              : kernel.pretty(sval[2]),
            Math.round(sval[3] * 10) / 10,
            sval[3] > 0 && sval[1] + sval[2] > 0
              ? Math.round((sval[3] / (sval[1] + sval[2])) * 10000) / 10
              : "N/A",
            "click sstep",
          ]),
        );
      ...
      str.push(
        $.format(trTpl, [
          ...
          (val[2] / maxSubt) * 100,
          isPercent
            ? Math.round((val[1] / sumSubt) * 1000) / 10 + "%"
            : kernel.pretty(val[1]),
          isPercent
            ? Math.round((val[2] / sumSubt) * 1000) / 10 + "%"
            : kernel.pretty(val[2]),
          Math.round(val[3] * 10) / 10,
          val[3] > 0 && val[1] + val[2] > 0
            ? Math.round((val[3] / (val[1] + val[2])) * 10000) / 10
            : "N/A",
    ...
    var endTr = $("<tr>").append(
      tidx
        ? $("<td>").append(requestBack)
        : $('<td style="padding:0;">').append(rangeSelect),
      "<td>" +
        (isPercent
          ? Math.round((totIns / sumSubt) * 1000) / 10 + "%"
          : kernel.pretty(totIns)) +
        "</td>" +
        "<td>" +
        (isPercent
          ? Math.round((totExec / sumSubt) * 1000) / 10 + "%"
          : kernel.pretty(totExec)) +
        "</td>",
      $("<td>").append(
        scramble || solve ? reconsClick : Math.round(totMov * 10) / 10,
      ),
      "<td>" +
        (totMov > 0 && totIns + totExec > 0
          ? Math.round((totMov / (totIns + totExec)) * 10000) / 10
          : "N/A") +
        "</td>",
    );
      ...
      reconsClick
        ...
        .attr(
          ...
          "https://alg.cubing.net/?alg=" +
            encodeURIComponent(solve) +
            "&setup=" +
            encodeURIComponent(scramble),
        )
        .text(Math.round(totMov * 10) / 10);
    }
  }
  ...
  function reqRecons(signal, value) {
    ...
    var solve = cubeutil.getPrettyReconstruction(
      rec.rawMoves,
      method,
    ).prettySolve;
  ...
  function showCasesDialog(method) {
    kernel.setProp(
      "rcCaseMthd",
      (method == "ZBLL" ? "cf3zb_" : "cf4op_") + method,
    );
    ...
    kernel.showDialog(
      [
        ...
        undefined,
        onCloseDialog,
        [EXPORT_TOFILE, exportCaseStatsData],
      ],
      "casestats",
      "Cases Stats",
    );
  ...
  $(function () {
    ...
    for (var i = 0; i < ranges.length; i++) {
      rangeSelect.append(
        '<option value="' + ranges[i] + '">' + ranges[i] + "</option>",
      );
    }
    var methods = [
      ["cf4op", "cfop"],
    ...
    for (var i = 0; i < methods.length; i++) {
      methodSelect.append(
        '<option value="' + methods[i][0] + '">' + methods[i][1] + "</option>",
      );
      methodSelect.append(
        '<option value="' +
          methods[i][0] +
    ...
    stats.regExtraInfo(
      ...
      [
        ...
        function (val) {
          return (
            "" +
            (val >= 0 ? val.toFixed(kernel.getProp("useMilli") ? 3 : 2) : "N/A")
          );
        },
      ],
    );
      ...
      [
        ...
        function (val) {
          return (
            "" +
            (val > 0
              ? (1e9 - val).toFixed(kernel.getProp("useMilli") ? 3 : 2)
              : "N/A")
          );
        },
      ],
...
var caseStat = execMain(function () {
  ...
  function update() {
    ...
    for (var i = ident[2]; i < ident[3]; i++) {
      ...
      maxSubt = Math.max(
        maxSubt,
        (caseCnts[i][1] + caseCnts[i][2]) / caseCnts[i][0],
      );
    }
  ...
  function procHeaderClick(e) {
    ...
    if (sortCol) {
      ...
      if (prevSortCol == sortCol) {
        kernel.setProp(
          "rcCaseSortDir",
          kernel.getProp("rcCaseSortDir") == "desc" ? "asc" : "desc",
        );
      }
      update();
    }
  ...
  $(function () {
    ...
    steps.forEach((step) => {
      methodStepSelect.append(
        `<option value="${step[0]}_${step[1]}">${step[1]}</option>`,
      );
      stats.regExtraInfo(
        `recons_${step[0]}_${step[1]}`,
        calcCaseExtra.bind(null, step[0], step[1]),
      );
    });
  });
...
var scatter = execMain(function () {
  ...
  function renderRecons(times, method) {
    ...
    for (var i = rec.data.length - 1; i >= 0; i--) {
      ...
      if (i != 0) {
        arcs.push([
          xs[0] * (width - offx) + offx,
          (1 - ys[0]) * (height - offy) + offy,
        ]);
      }
    }
    ...
    slowSpan.html(
      " " +
        spanT
          .replace("$", "#f00")
          .replace("$", kernel.pretty(slow0 * finalTime)) +
        " " +
        spanT
          .replace("$", "#00f")
          .replace("$", kernel.pretty(slow1 * finalTime)) +
        " " +
        spanT
          .replace("$", "#888")
          .replace("$", kernel.pretty((1 - slow0 - slow1) * finalTime)),
    );
  }
  ...
  function plot(x, y, color) {
    ...
    ctx.moveTo(
      x[0] * (width - offx) + offx,
      (1 - y[0]) * (height - offy) + offy,
    );
    for (var i = 1; i < x.length; i++) {
      ctx.lineTo(
        x[i] * (width - offx) + offx,
        (1 - y[i]) * (height - offy) + offy,
      );
    }
    ctx.stroke();
    ctx.closePath();
  ...
  function execFunc(fdiv, signal) {
    ...
    var span =
      '<span class="click" data="%" style="font-family: iconfont, Arial;display:inline-block;width:2em;">$</span>';
    fdiv.empty().append(
      methodSelect,
      slowSpan,
      scatterDiv
