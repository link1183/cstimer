var trainStat = execMain(function () {
  ...
  var tableTh = $("<tr>").append(
    $('<th colspan=2 style="padding:0;">').append(methodSelect),
  ...
  function update() {
    ...
    for (var i = ident[2]; i < ident[3]; i++) {
      ...
      var statObj = new TimeStat(
        [],
        caseCnts[i].length,
        timeMapAt.bind(null, caseCnts[i]),
      );
  ...
  $(function () {
    ...
    for (var i = 0; i < methods.length; i++) {
      ...
      stats.regExtraInfo(
        "scramcase_" + methods[i],
        calcCaseExtra.bind(null, methods[i]),
      );
    }
  });
});
