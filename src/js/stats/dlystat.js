...
var periodStats = execMain(function () {
    ...
    switch (period) {
      ...
      case "w":
        return mathlib.time2str(
          (pidx * 7 + ~~sowSelect.val()) * 86400 + offset,
          "Start@ %Y-%M-%D",
        );
      case "m":
  ...
  function genDiv() {
    ...
    toolDiv
      ...
      .append(
        dlyStr[0],
        dateSelect.unbind("change").change(update),
        " " + dlyStr[1],
        sodSelect.unbind("change").change(update),
        " " + dlyStr[2],
        sowSelect.unbind("change").change(update),
        "<br>",
        table,
      );
    ...
    var tr0 = $("<tr>").append(
      prevTd.unbind("click").click(procClick),
      nextTd.unbind("click").click(procClick),
    );
    var tr1 = $("<tr>").append(
      incTd.unbind("click").click(procClick),
      decTd.unbind("click").click(procClick),
    );
    for (var i = 0; i < n_col; i++) {
      tr0.append(
        $("<td rowspan=2>").html(
          pidx2str(curPidx - i + idxPrev).replace(" ", "<br>"),
        ),
      );
    }
    ...
    for (var i = 0; i < sessionN; i++) {
      ...
      var tr = $("<tr>").append(
        $("<td colspan=2>").html(sessionData[idx]["name"]),
      );
