...
var TimeStat = execMain(function () {
  ...
  TimeStat.prototype.getAllStats = function () {
    ...
    return [
      numdnf,
      numdnf == this.timesLen
        ? -1
        : this.tree.avgstd(0, this.timesLen - numdnf)[0],
    ];
  };
  ...
  TimeStat.prototype.doPushed = function (silent, next) {
    ...
    for (var j = 0; j < this.avgSizes.length; j++) {
      ...
      var curVal = [
        rbt.rankOf(-1) < size - trimlr[1] ? -1 : avgstd[0],
        avgstd[1] / 1000,
        rbt.rank(trimlr[0] - 1),
        rbt.rank(size - trimlr[1]),
      ];
  ...
  TimeStat.prototype.getThres = function () {
    ...
    for (var j = 0; j < this.avgSizes.length; j++) {
      ...
      if (rbt.rankOf(-1) <= right) {
        //next avg is always DNF
        thres[j] = -1;
        continue;
      } else if (tgtAvg == -1) {
        thres[j] = -2;
  ...
  TimeStat.prototype.getBWPA = function () {
    ...
    for (var j = 0; j < this.avgSizes.length; j++) {
      ...
      rbt.insert(0, 0);
      bpa[j] =
        rbt.rankOf(-1) < size - trimlr[1]
          ? -1
          : rbt.avgstd(trimlr[0], size - trimlr[1])[0];
  ...
  TimeStat.prototype.getTrimList = function (start, nsolves, thresL, thresR) {
    ...
    return trimlList
      .slice(trimlList.length - trimlr[0])
      .concat(trimrList.slice(trimrList.length - trimlr[1]));
  };
