...
var stats = execMain(
  function (kpretty, round, kpround) {
    ...
    var avgRow = $("<tr>");
    var showAllRow = $(
      '<tr class="click" ><th class="click" colspan="15">...</th></tr>',
    );

    var sumtable = $('<table class="sumtable">')
      .click(function (e) {
    ...
    function push(time) {
      if (typeof time[0] == "string") {
        var val = [
          ...
          time[1] || curScramble,
          time[0],
          time[3] || Math.round((new Date().getTime() - time[2][1]) / 1000),
        ];
        if (time[4]) {
          val.push(time[4]);
        }
        ...
        times.push([
          ...
          curScramble,
          "",
          Math.round((new Date().getTime() - time[1]) / 1000),
        ]);
      }
    ...
    function pretty(time, showDNF) {
      switch (time[0]) {
        case 0:
          return kpretty(time[1]);
        case -1:
          return "DNF" + (showDNF ? `(${kpretty(time[1])})` : "");
        default:
          return kpretty(time[0] + time[1]) + "+";
      }
    }

    function prettyMPA(time) {
      // multi phase append, e.g. "=XXX+XXX+XXX..."
      if (time.length == 2) {
        return "";
      }
      var ret = [];
    ...
    var table_ctrl = (function () {
      ...
      function changeRank(idx) {
        ...
        if (idx == 0) {
          rank.sort(function (a, b) {
            return TimeStat.dnfsort(
              times_stats_table.timeAt(a),
              times_stats_table.timeAt(b),
            );
          });
        } else if (idx == 1 || idx == 2) {
          var len = idx == 1 ? len1 : len2;
          var runstat = times_stats_list.runAvgMean(
            0,
            times.length,
            len,
            (idx == 1 ? stat1 : stat2) > 0 ? undefined : 0,
          );
          rank.sort(function (a, b) {
      ...
      function filter(idx) {
        ...
        return (
          pattern.exec(pretty(time[0], true) + prettyMPA(time[0])) ||
          pattern.exec(time[1]) ||
          pattern.exec(time[2]) ||
      ...
      function updateTable(allUpdate) {
        ...
        scrollDiv.scrollTop(
          kernel.getProp("statinv") ? table[0].scrollHeight : 0,
        );
      }

      function changePattern() {
        var newpattern = $.prompt(
      ...
      function updateTitleRow() {
        ...
        len1Th.html(
          (stat1 > 0 ? "ao" : "mo") + len1 + (rankIdx == 1 ? "*" : ""),
        );
        len2Th.html(
          (stat2 > 0 ? "ao" : "mo") + len2 + (rankIdx == 2 ? "*" : ""),
        );
      ...
      function getRowIndexOf(idx) {
        ...
        var ii = kernel.getProp("statinv")
          ? shownIdxs.length + 1 - sidx
          : 2 + sidx;
    ...
    var floatCfm = (function () {
      ...
      function procClk(e) {
        ...
        if (which == "p") {
          var selected = { " OK ": 0, " +2 ": 2000, " DNF ": -1 }[
            target.html()
          ];
          ...
          if (time[4]) {
            var puzzle =
              (typeof time[4][1] == "string" && time[4][1]) ||
              tools.getCurPuzzle() ||
              "333";
      ...
      function genDiv() {
        ...
        if (time[4]) {
          ...
          reviewElem = $("<tr>").append(
            $("<td>").append(reviewElem),
            $("<td>").append(cfmExt),
          );
        }
        cfmDiv
          .empty()
          .append(cfmTime, "<br>", prettyMPA(time[0]), "<br>")
          .append(
    ...
    function procAvgClick(e) {
      ...
      if (idx != 0) {
        stats = new TimeStat(
          avgSizes,
          times.length,
          timeAtDim.bind(undefined, idx),
        );
      }
      getStats(
        stats,
        timesAt,
        idx == 0 ? 0 : STATS_CURSPLIT.replace("%d", idx),
      );
    }
    ...
    function getTimeRow(i, dim, tr) {
      ...
      if (statSrc[0] != "t") {
        ...
        ret.push(
          (st0pb ? tdpb : tdtm) +
            ...
            pretty(curTime, false) +
            "</span> " +
            prettyFunc[0](times_stats_table.timeAt(i)).split(" ")[0] +
            "</td>",
        );
      }

      var st1 = times_stats_table.runAvgMean(
        i - len1 + 1,
        len1,
        0,
        stat1 > 0 ? undefined : 0,
      );
      var st2 = times_stats_table.runAvgMean(
        i - len2 + 1,
        len2,
        0,
        stat2 > 0 ? undefined : 0,
      ...
      ret.push(
        (st1 ? (st1pb ? tdpb : tdtm) + prettyFunc[1](st1[0][0]) : "<td>-") +
          "</td>" +
          (st2 ? (st2pb ? tdpb : tdtm) + prettyFunc[1](st2[0][0]) : "<td>-") +
          "</td>",
      );
      if (dim > 1) {
    ...
    function updateAvgRow(dim) {
      ...
      avgRow
        .append(
          `<th colspan="4" data="0" class="times">${STATS_SOLVE}: ${len - data[0]}/${len}<br>` +
            STATS_AVG +
            ": " +
            prettyFunc[1](data[1]) +
            (kernel.getProp("statssum")
              ? `<br>${STATS_SUM}: ${prettyFunc[0](sum)}`
              : "") +
            "</th>",
        )
        .css("font-size", "1.2em");
      if (dim > 1) {
        for (var j = 1; j <= dim; j++) {
          avgRow
            .append(`<th data="${j}" class="times">${kpround(getMean(j))}</th>`)
            .css("font-size", "");
        }
      }
    }
    ...
    function genSrcSelect(statSrcSelect) {
      ...
      for (var i = 0; i < validOpt.length; i++) {
        statSrcSelect.append(
          $("<option>").val(validOpt[i][0]).html(validOpt[i][1]),
        );
    ...
    function updateSumTable() {
      ...
      if (times.length > 0) {
        ...
        shead.push(
          '<td class="times click" data="cs">' +
            prettyFunc[0](times_stats_table.timeAt(idx)) +
            "</td>",
        );
        shead.push(
          '<td class="times click" data="bs">' +
            prettyFunc[0](times_stats_table.bestTime) +
            "</td>",
        );
      } else {
      ...
      for (var j = 0; j < avgSizes.length; j++) {
        ...
        var rowhead =
          "<tr><th>" + "am"[avgSizes[j] >>> 31] + "o" + size + "</th>";
        if (times.length >= size) {
          ...
          s.push(
            '<td class="times click" data="c' +
              ...
              j +
              '">' +
              prettyFunc[1](times_stats_table.lastAvg[j][0]) +
              "</td>",
          );
          s.push(
            '<td class="times click" data="b' +
              ...
              j +
              '">' +
              prettyFunc[1](times_stats_table.bestAvg(j, 0)) +
              "</td>",
          );
          showThres &&
            s.push(
        ...
        showBPA &&
          s.push('<td class="times">' + prettyFunc[1](bwpa[0][j]) + "</td>");
        showWPA &&
          s.push(
            avgSizes[j] < 0
              ? "<td>-</td>"
              : '<td class="times">' + prettyFunc[1](bwpa[1][j]) + "</td>",
          );
        s.push("</tr>");
      }
      var curTh = $("<tr>").append(
        "<th></th><th>" + hlstr[1] + "</th><th>" + hlstr[0] + "</th>",
      );
      if (showThres) {
    ...
    function detailTimeLine(idx, time, trimList) {
      ...
      var c =
        pretty(time[0], true) +
        prettyMPA(time[0]) +
        (kernel.getProp("printComm") && time[2] ? "[" + time[2] + "]" : "");
    ...
    function setHighlight(times_stats, timesAt, start, nsolves, id, mean) {
      ...
      if (kernel.getProp("printDate") && nsolves > 2) {
        ...
        tstr = hlstr[11]
          .replace("%s", mathlib.time2str(tstart && tstart[3]))
      ...
      for (var i = 0; i < nsolves; i++) {
        s.push(
          detailTimeLine(
            kernel.getProp("absidx") ? start + i : i,
            timesAt(start + i),
            trimList,
          ),
        );
    ...
    function exportCSV(times_stats, timesAt, start, nsolves) {
      ...
      outFile.attr(
        "download",
        "csTimerExport_" +
          mathlib.time2str(new Date() / 1000, "%Y%M%D_%h%m%s") +
          ".csv",
      );
      outFile[0].click();
      outFile.remove();
      ...
      switch (which.substr(0, 2)) {
        ...
        case "cm":
          setHighlight(
            times_stats,
            timesAt,
            times_stats.timesLen + avgSizes[idx],
            -avgSizes[idx],
            -avgSizes[idx] * 10,
            true,
          );
    ...
    function timeAtDim(dim, idx) {
      ...
      var ret =
        dim == 0
          ? curTime[0] + curTime[1]
          : curTime[curTime.length - dim] -
            (curTime[curTime.length - dim + 1] || 0);
    ...
    var sessionManager = (function () {
      ...
      function fixSessionSelect() {
        ...
        for (var i = 0; i < ssSorted.length; i++) {
          select.append(
            $("<option>")
              .val(ssSorted[i])
              .html(sessionData[ssSorted[i]]["name"]),
          );
        }
      ...
      function initNewSession(rank, copy) {
        ...
        var newName =
          curDate.getMonth() + 1 + "." + curDate.getDate() + " " + curScrType;
      ...
      function deleteSession(ssidx) {
        if (
          ("stat" in sessionData[ssidx] ? sessionData[ssidx]["stat"][0] : 1) !=
            0 &&
      ...
      function renameSession(ssidx, isCreate) {
        ...
        var sName = $.prompt(
          isCreate ? STATS_SESSION_NAMEC : STATS_SESSION_NAME,
          sessionData[ssidx]["name"],
        );
        if (sName != null) {
      ...
      function sessionLoaded(loadedIdx, timesNew) {
        if (loadedIdx != sessionIdx) {
          // session changed when loading, abort
          return;
        }
        isInit = false;
        times = timesNew;
        ...
        sessionData[loadedIdx]["stat"] = [times.length].concat(
          times_stats_list.getAllStats(),
        );
        sessionData[loadedIdx]["date"] = [
          (times[0] || [])[3],
          (times.at(-1) || [])[3],
        ];
        kernel.setProp("sessionData", JSON.stringify(sessionData));
      ...
      function load() {
        return storage
          .get(sessionIdx)
          .then(sessionLoaded.bind(undefined, sessionIdx));
      }

      function save(startIdx) {
        sessionData[sessionIdx]["stat"] = [times.length].concat(
          times_stats_list.getAllStats(),
        );
        sessionData[sessionIdx]["date"] = [
          (times[0] || [])[3],
          (times.at(-1) || [])[3],
        ];
        kernel.setProp("sessionData", JSON.stringify(sessionData));
      ...
      function mgrClick(e) {
        var target = $(e.target);
        if (
          !target.is("td, th, select") ||
          (!target.hasClass("click") && !target.is("select"))
        ) {
          return;
        }
            ...
            storage.get(idx).then(function (newTimes) {
              exportCSV(
                new TimeStat(
                  ...
                  newTimes.length,
                  function (times, idx) {
                    return times[idx][0][0] == -1
                      ? -1
                      : ~~((times[idx][0][0] + times[idx][0][1]) / roundMilli) *
                          roundMilli;
                  }.bind(undefined, newTimes),
                ),
                function (times, idx) {
                  return times[idx];
                }.bind(undefined, newTimes),
      ...
      function splitSession() {
        var n_split = $.prompt(
          STATS_PROMPTSPL.replace("%s", sessionIdent(sessionIdx)),
          ~~(times.length / 2),
        );
        if (n_split == null) {
          return;
      ...
      function mergeSessionTo(idx, dedup) {
        if (
          sessionIdx == idx ||
          !$.confirm(
            STATS_ALERTMG.replace("%f", sessionIdent(sessionIdx)).replace(
              "%t",
              sessionIdent(idx),
            ),
          )
      ...
      function getMgrRowAtRank(rank) {
        ...
        var sel =
          ...
          "</option>" +
          '<option value="' +
          (idx == sessionIdx ? 'p">' + ops[2] : 'm">' + ops[3]) +
          "</option>" +
          '<option value="' +
          (idx == sessionIdx ? 'o">' + ops[5] : 'md">' + ops[6]) +
          "</option>" +
          '<option value="x">' +
          ops[4] +
          "</option>" +
        ...
        var dClk =
          rank == ssSorted.length
            ? "<td></td>"
            : '<td class="click" data="d">&#8595;</td>';
        var scrTd =
          "<td>" +
          scramble.getTypeName(ssData["opt"]["scrType"] || "333") +
          "</td>";
        var ssTd0 = "<td>" + ssStat[0] + "</td>";
        var ssTd1 = "<td>" + ssStat[1] + "</td>";
        var dateVal = mathlib.time2str(
          (sessionData[idx]["date"] || [])[1],
          "%Y-%M-%D",
        );
        return (
          '<tr class="' +
          (idx == sessionIdx ? "selected mhide" : "mhide") +
          '">' +
          ...
          "</tr>" +
          '<tr class="' +
          (idx == sessionIdx ? "selected " : "") +
          'mshow t">' +
          '<td class="click" data="s" rowspan=2>' +
          rank +
          "-" +
          ssData["name"] +
          (idx == sessionIdx ? "*" : "") +
      ...
      function getMgrRowAtGroup(group) {
        ...
        for (var i = 0; i < group.length; i++) {
          ...
          ssNames.push(
            sessionData[idx]["name"] +
              "(" +
              scramble.getTypeName(
                sessionData[idx]["opt"]["scrType"] || "333",
              ) +
              ")",
          );
        }
        ...
        ssmgrTable
          ...
          .append(
            '<tr class="mhide"><th class="click" data=' +
              ...
              '<tr class="mshow t"><th rowspan=2 class="click" data=' +
              (byGroup == "name" ? '"g">[+]' : '"gn">[-]') +
              " " +
              STATS_SSMGR_NAME +
              "</th><th>" +
          ...
          [
            ...
            [
              ...
              function () {
                ...
                ssSorted.sort(function (a, b) {
                  var idxa = scramble.getTypeIdx(
                    sessionData[a]["opt"]["scrType"] || "333",
                  );
                  var idxb = scramble.getTypeIdx(
                    sessionData[b]["opt"]["scrType"] || "333",
                  );
                  return idxa == idxb
                    ? sessionData[a]["rank"] - sessionData[b]["rank"]
                    : idxa - idxb;
                });
                for (var i = 0; i < ssSorted.length; i++) {
                  sessionData[ssSorted[i]]["rank"] = i + 1;
    ...
    function getStats(times_stats, timesAt, title) {
      ...
      s.push(
        "    " + hlstr[2] + ": " + prettyFunc[0](times_stats.worstTime) + "\n",
      );
      for (var j = 0; j < avgSizes.length; j++) {
        var size = Math.abs(avgSizes[j]);
        if (length >= size) {
          ...
          s.push(
            "    " +
              hlstr[1] +
              ": " +
              prettyFunc[1](times_stats.lastAvg[j][0]) +
              " (σ = " +
              trim(times_stats.lastAvg[j][1], 2) +
              ")",
          );
          s.push(
            "    " +
              hlstr[0] +
              ": " +
              prettyFunc[1](times_stats.bestAvg(j, 0)) +
              " (σ = " +
              trim(times_stats.bestAvg(j, 1), 2) +
              ")\n",
          );
        }
      }
      ...
      s.push(
        hlstr[8]
          .replace("%v", prettyFunc[1](sessionavg[0]))
          .replace("%sgm", trim(sessionavg[1], 2))
          .replace(/[{}]/g, ""),
      );
    ...
    function updateStatalU(statal) {
      ...
      times_stats_table = new TimeStat(
        avgSizes,
        times.length,
        getTableTimeAt(),
      );
    ...
    function resultsHeight() {
      if ($("html").hasClass("m")) {
        scrollDiv.height(
          Math.max(sumtableDiv.height(), avgRow.height() + title.height() * 2),
        );
      } else if (scrollDiv[0].offsetParent != null) {
        scrollDiv.outerHeight(
          ~~(
            div.height() -
            (statOptDiv.is(":hidden") ? 0 : statOptDiv.outerHeight()) -
            sumtableDiv.outerHeight() -
            5
          ),
        );
      }
    ...
    $(function () {
      ...
      kernel.regListener(
        ...
        "property",
        procSignal,
        /^(:?useMilli|timeFormat|stat(:?sum|thres|[bw]pa|[12][tl]|alu?|inv|Hide|src|ssum)|session(:?Data)?|scrType|phases|trimr?|view|wndStat|sr_.*)$/,
      );
      ...
      div
        .append(
          statOptDiv.append(
            $('<span class="click">')
              .html(STATS_SESSION)
              .click(sessionManager.showMgrTable),
            sessionManager.getSelect(),
            sessionManager.getButton(),
          ),
      ...
      scrollDiv.bind("scroll", function () {
        ...
        if (
          elem.scrollHeight - elem.scrollTop < elem.clientHeight + 5 &&
          !kernel.getProp("statinv")
        ) {
          showAllRow.click();
        }
      ...
      kernel.regProp(
        ...
        "stat1t",
        1,
        stattl[0].replace("%d", 1),
        [0, [0, 1], stattl.slice(2)],
        1,
      );
      kernel.regProp(
        "stats",
    ...
    for (var i = 0; i < 5; i++) {
      regExtraInfo("comment" + i, getCommentNumber.bind(null, i), [
        STATS_COMMENT + (i + 1),
        function (val) {
          return (
            "" +
        ...
        function (val) {
          return (
            "" +
            (val >= 0
              ? (val * 1e-3).toFixed(kernel.getProp("useMilli") ? 3 : 2)
              : "DNF")
          );
        },
      ]);
      ...
      [
        ...
        function (val) {
          ...
          return (
            "" +
            (val >= 0
              ? score.toFixed(kernel.getProp("useMilli") ? 3 : 2)
              : "DNF")
          );
        },
      ],
