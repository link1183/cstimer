...
execMain(function () {
  ...
  function getManufacturerDataBytes(mfData) {
    ...
    for (var id of QIYI_CIC_LIST) {
      if (mfData.has(id)) {
        giikerutil.log(
          "[QiyiTimer] found Manufacturer Data under CIC = 0x" +
  ...
  function onReadEvent(event) {
    ...
    if (
      crc16modbus(
        data.slice(0, len + 12).concat([data[len + 13], data[len + 12]]),
      ) != 0
    ) {
  ...
  function init(device) {
    ...
    decoder = decoder || $.aes128(Array(16).fill(0x77));
    return BluetoothTimer.waitForAdvs()
      ...
      .then(
        function (mac) {
          giikerutil.log(
            "[QiyiTimer] init, found cube bluetooth hardware MAC = " + mac,
          );
          deviceMac = mac;
        },
        function (err) {
          giikerutil.log(
            "[QiyiTimer] init, unable to automatically determine cube MAC, error code = " +
              err,
          );
        },
      )
      ...
      .then(function (chrcts) {
        ...
        if (!readChrct || !writeChrct) {
          return Promise.reject(
            "[QiyiTimer] Cannot find required characteristics",
          );
        }
        giikerutil.log(
          "[QiyiTimer] start listening to state characteristic value updates",
        );
      ...
      .then(function () {
        ...
        if (m) {
          defaultMac =
            (deviceName.startsWith("QY-Adapter") ? "CC:A8" : "CC:A1") +
            ":00:00:" +
            m[1].slice(0, 2) +
            ":" +
            m[1].slice(2, 4);
