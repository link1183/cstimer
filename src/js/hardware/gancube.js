execMain(function () {
  ...
  var GAN_CIC_LIST = mathlib.valuedArray(256, function (i) {
    return (i << 8) | 0x01;
  });
  ...
  function v3init() {
    ...
    return _service_v3data
      ...
      .then(function () {
        ...
        return _chrct_v3read.addEventListener(
          "characteristicvaluechanged",
          onStateChangedV3,
        );
      })
  ...
  function v4init() {
    ...
    return _service_v4data
      ...
      .then(function () {
        ...
        return _chrct_v4read.addEventListener(
          "characteristicvaluechanged",
          onStateChangedV4,
        );
      })
  ...
  function checkState() {
    ...
    return _chrct_f2.readValue().then(function (value) {
      ...
      for (var i = 0; i < value.length - 2; i += 3) {
        var face =
          (value[i ^ 1] << 16) | (value[(i + 1) ^ 1] << 8) | value[(i + 2) ^ 1];
  ...
  function updateMoveTimes(locTime, isV2) {
    ...
    for (var i = moveDiff - 1; i >= 0; i--) {
      var m =
        "URFDLB".indexOf(prevMoves[i][0]) * 3 + " 2'".indexOf(prevMoves[i][1]);
  ...
  function parseV2Data(value) {
    ...
    if (mode == 1) {
      ...
      var hardwareVersion =
        parseInt(value.slice(8, 16), 2) +
        "." +
        parseInt(value.slice(16, 24), 2);
      var softwareVersion =
        parseInt(value.slice(24, 32), 2) +
        "." +
        parseInt(value.slice(32, 40), 2);
      var devName = "";
      for (var i = 0; i < 8; i++)
  ...
  function parseV3Data(value) {
    ...
    if (mode == 1) {
      ...
      giikerutil.log(
        "[gancube]",
        "v3 facelets event state parsed",
        latestFacelet,
      );
      initCubeState();
    } else if (mode == 6) {
      ...
      var hardwareVersion =
        parseInt(value.slice(80, 84), 2) +
        "." +
        parseInt(value.slice(84, 88), 2);
      var softwareVersion =
        parseInt(value.slice(72, 76), 2) +
        "." +
        parseInt(value.slice(76, 80), 2);
      var devName = "";
      for (var i = 0; i < 5; i++)
        devName += String.fromCharCode(
          parseInt(value.slice(32 + i * 8, 40 + i * 8), 2),
        );
      giikerutil.log("[gancube]", "Hardware Version", hardwareVersion);
      giikerutil.log("[gancube]", "Software Version", softwareVersion);
      giikerutil.log("[gancube]", "Device Name", devName);
  ...
  function parseV4Data(value) {
    ...
    if (mode == 0x01) {
      ...
      giikerutil.log(
        "[gancube]",
        "v4 facelets event state parsed",
        latestFacelet,
      );
      initCubeState();
    } else if (mode == 0xd1) {
        ...
        case 0xfc:
          ...
          for (var i = 0; i < len - 1; i++)
            hwName += String.fromCharCode(
              parseInt(value.slice(24 + i * 8, 32 + i * 8), 2),
            );
  ...
  function clear() {
    ...
    if (_chrct_v3read) {
      _chrct_v3read.removeEventListener(
        "characteristicvaluechanged",
        onStateChangedV3,
      );
    ...
    if (_chrct_v4read) {
      _chrct_v4read.removeEventListener(
        "characteristicvaluechanged",
        onStateChangedV4,
      );
