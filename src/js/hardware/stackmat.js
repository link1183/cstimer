...
var stackmat = execMain(function () {
  ...
  function success(stream) {
    ...
    node["onaudioprocess"] = function (e) {
      ...
      for (var i = 0; i < input.length; i++) {
        ...
        last_power = Math.max(
          0.0001,
          last_power + (power - last_power) * agc_factor,
        );
  ...
  function procSignal(signal) {
    ...
    var isEdge =
      (lastVal.pop() - signal) * (lastSgn ? 1 : -1) > THRESHOLD_EDGE &&
    ...
    if (last_bit_length < 10) {
      distortionStat = Math.max(
        0.0001,
        distortionStat +
          (Math.pow(signal - (lastSgn ? 1 : -1), 2) - distortionStat) *
            agc_factor,
      );
  ...
  function decode(byteBuffer) {
    ...
    var time_milli =
      ...
      ~~(
        byteBuffer[4] +
        byteBuffer[5] +
        (byteBuffer.length == 10 ? byteBuffer[6] : "0")
      );
