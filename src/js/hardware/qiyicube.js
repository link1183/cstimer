execMain(function () {
  ...
  function sendMessage(content) {
    ...
    decoder =
      decoder ||
      $.aes128(JSON.parse(LZString.decompressFromEncodedURIComponent(KEYS[0])));
  ...
  function init(device) {
    ...
    return GiikerCube.waitForAdvs()
      ...
      .then(
        function (mac) {
          giikerutil.log(
            "[qiyicube] init, found cube bluetooth hardware MAC = " + mac,
          );
          deviceMac = mac;
        },
        function (err) {
  ...
  function parseCubeData(msg) {
    ...
    if (opcode == 0x2) {
      ...
      if (todoMoves.length > 1) {
        giikerutil.log(
          "[qiyicube] miss history moves",
          JSON.stringify(todoMoves),
          lastTs,
        );
      }
      var toCallback = [];
  ...
  function clear() {
    ...
    if (_chrct_cube) {
      _chrct_cube.removeEventListener(
  ...
  GiikerCube.regCubeModel({
    ...
    getBatteryLevel: function () {
      return Promise.resolve([batteryLevel, _deviceName]);
    },
    clear: clear,
  });
});
