...
function BtDeviceGroupFactory() {
  ...
  function init(reconnect) {
    return giikerutil
      ...
      .then(function () {
        ...
        var filters = Object.keys(cubeModels).map((prefix) => ({
          namePrefix: prefix,
        }));
        var opservs = [
          ...new Set(
            Array.prototype.concat.apply(
              [],
              Object.values(cubeModels).map(
                (cubeModel) => cubeModel.opservs || [],
              ),
            ),
          ),
        ];
      ...
      .then(function (device) {
        giikerutil.log(
          "[bluetooth]",
          "BLE device is selected, name=" + device.name,
          device,
        );
        _device = device;
    ...
    return new Promise(function (resolve, reject) {
      if (!device.watchAdvertisements) {
        ...
        var onAdvEvent = function (event) {
          DEBUG &&
            console.log(
              "[bluetooth] received advertisement packet from device",
              event,
            );
        ...
        device.stopWaiting = function () {
          DEBUG &&
            console.log("[bluetooth] cancel waiting for device advertisements");
          delete device.stopWaiting;
          device.removeEventListener("advertisementreceived", onAdvEvent);
          abortController.abort();
        };
        ...
        DEBUG &&
          console.log(
            "[bluetooth] start waiting for device advertisement packet",
          );
      }
    });
  }
  ...
  function stop(isHardwareEvent) {
    ...
    return Promise.resolve(cube && cube.clear(isHardwareEvent)).then(
      function () {
