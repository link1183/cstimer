execMain(function () {
  ...
  // CICs 0x(01..=FF)00
  var MOYU32_CIC_LIST = mathlib.valuedArray(255, function (i) {
    return (i + 1) << 8;
  });
  ...
  function initMac(forcePrompt, isWrongKey) {
    ...
    deviceMac = giikerutil.reqMacAddr(
      forcePrompt,
      isWrongKey,
      deviceMac,
      defaultMac,
  ...
  function init(device) {
    ...
    return GiikerCube.waitForAdvs()
      ...
      .then(
        function (mac) {
          giikerutil.log(
            "[Moyu32Cube] init, found cube bluetooth hardware MAC = " + mac,
          );
          deviceMac = mac;
        },
        function (err) {
      ...
      .then(function (chrcts) {
        ...
        if (!_chrct_read) {
          return Promise.reject(
            "[Moyu32Cube] Cannot find required characteristics",
          );
        }
        _chrct_read.addEventListener(
    ...
    if (msgType == 161) {
      ...
      for (var i = 0; i < 8; i++)
        devName += String.fromCharCode(
          parseInt(value.slice(8 + i * 8, 16 + i * 8), 2),
        );
      var hardwareVersion =
        parseInt(value.slice(88, 96), 2) +
        "." +
        parseInt(value.slice(96, 104), 2);
      var softwareVersion =
        parseInt(value.slice(72, 80), 2) +
        "." +
        parseInt(value.slice(80, 88), 2);
  ...
  function updateMoveTimes(locTime) {
    ...
    moveDiff > 1 &&
      giikerutil.log(
        "[Moyu32Cube] bluetooth event was lost, moveDiff = " + moveDiff,
      );
    prevMoveCnt = moveCnt;
  ...
  function clear() {
    ...
    _service = null;
    if (_chrct_read) {
      _chrct_read.removeEventListener(
        "characteristicvaluechanged",
        onStateChanged,
      );
