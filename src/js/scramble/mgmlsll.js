(function () {
  ...
  function getMinxLSScramble(type, length, cases) {
    ...
    do {
      if (type == "mlsll") {
        ...
        co[5] = 0;
        edge =
          epcord.get(mathlib.rndPerm(5, true).concat([5])) * 32 +
          eocord.get(eo);
        corn =
          cpcord.get(mathlib.rndPerm(5, true).concat([5])) * 243 +
          cocord.get(co);
      }
    } while (edge == 0 && corn == 0);
  ...
  scrMgr.reg("mlsll", getMinxLSScramble)("mgmpll", getMinxLSScramble)(
    "mgmll",
    getMinxLSScramble,
  );
})();
