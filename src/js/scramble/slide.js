  ...
  function getScramble(size, type) {
    ...
    if (size == 4) {
      ...
      var sol2 = solv2.search(
        stateInit("01234???8???c??-", perm[midx]).replace(/[0123]/g, "$"),
        0,
      );
      sol = sol.concat(sol2);
    } else if (size == 3) {
    ...
    var sol3 = solv3.search(
      stateInit("0123456789abcde-", perm[midx]).replace(/[012348c]/g, "$"),
      0,
    );
    sol = sol.concat(sol3);
    DEBUG &&
      console.log(
        "[15p solver]",
        midx,
        stateInit("0123456789abcde-", perm[midx]),
        sol.join(""),
        sol.length,
        +new Date() - t,
      );
