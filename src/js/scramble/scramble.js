...
var scrMgr = (function (rn, rndEl) {
  ...
  function MillerShuffle(idx, permIdx, length) {
    ...
    if (sidx % 3 == 0)
      sidx = (((sidx / 3) * p1 + r1) % ~~((length + 2) / 3)) * 3;
    if (sidx % 2 == 0)
      sidx = (((sidx / 2) * p2 + r2) % ~~((length + 1) / 2)) * 2;
    ...
    function (rn, rndEl) {
      ...
      function procLastClick() {
        ...
        if (lastscramble != undefined) {
          kernel.pushSignal(
            "scrambleX",
            scrStd(lasttype, lastscramble, lastlen),
          );
        }
      }
      ...
      function scrStd(type, scramble, len, forDisplay, forceKeyM) {
        ...
        if (forDisplay) {
          var fontSize = kernel.getProp("scrASize")
            ? Math.max(
                0.25,
                Math.round(
                  Math.pow(50 / Math.max(scrTxt.length, 10), 0.3) * 20,
                ) / 20,
              )
            : 1;
          ...
          return scramble
            ...
            .replace(
              /`(.*?)`/g,
              forceKeyM || kernel.getProp("scrKeyM", false)
                ? "<u>$1</u>"
                : "$1",
            );
        } else {
        ...
        if (csTimerWorker && csTimerWorker.getScramble) {
          cacheTid =
            ...
            csTimerWorker.getScramble(
              args,
              function (detailType, scramble) {
                DEBUG &&
                  console.log(
                    "[scrcache]",
                    detailType + " cached by csTimerWorker",
                  );
          ...
          cacheTid =
            cacheTid ||
            setTimeout(
              function (detailType, args) {
                var scrambler = scramblers[args[0]];
                saveCachedScramble(
                  detailType,
                  scrambler.apply(scrambler, args),
                );
              }.bind(undefined, detailType, args),
              500,
            );
        }
      }
      ...
      $(function () {
        ...
        for (var i = 0; i < forceCached.length; i++) {
          if (!(forceCached[i] in cachedScr)) {
            setTimeout(
              genCachedScramble.bind(
                undefined,
                JSON.parse(forceCached[i]),
                forceCached[i],
                true,
              ),
      ...
      function calcScramble() {
        ...
        if (realType in scramblers) {
          ...
          if (
            enableCache &&
            detailType in cachedScr &&
            !realType.startsWith("nocache_")
          ) {
            scramble = cachedScr[detailType];
            delete cachedScr[detailType];
            localStorage["cachedScr"] = JSON.stringify(cachedScr);
          } else {
            scramble = scramblers[realType](
              realType,
              len,
              rndState(scrFlt[1], getExtra(realType, 1)),
              neutLevel,
            );
          }
          genCachedScramble(
      ...
      var inputScrambleGen = (function () {
        ...
        function init() {
          ...
          for (var i = 0; i < puzzles.length; i++) {
            puzzleSelect.append(
              $("<option>").val(puzzles[i]).html(menu.getValName(puzzles[i])),
            );
          }
          inputDiv.append(
            $("<tr height=0%>").append(
              $("<td>").append(SCRAMBLE_INPUTTYPE + ":", puzzleSelect),
            ),
      ...
      function loadScrOpts() {
        ...
        scrFlt = JSON.parse(
          kernel.getProp(
            "scrFlt",
            JSON.stringify([curType, getExtra(curType, 0)]),
          ),
        );
        ...
        if (scrFlt[0] != curType) {
          scrFlt = [
            curType,
            getExtra(curType, 0) &&
              mathlib.valuedArray(getExtra(curType, 0).length, 1),
          ];
      ...
      function showScrOpt() {
        ...
        kernel.showDialog(
          [scrOptDiv, procDialog, null, procDialog],
          "scropt",
          SCROPT_TITLE,
        );
      }
      ...
      function procSignal(signal, value) {
        if (signal == "time") {
          ...
          if (value[0] == "scrSize") {
            ...
            div.css(
              "text-align",
              { c: "center", l: "left", r: "right" }[value[1]],
            );
            ...
            sdiv.html(
              isDisplayLast
                ? scrStd(lasttype, lastscramble || "", lastlen || 0, true)
                : scrStd(type, scramble || "", len, true),
            );
      ...
      var scrambleGenerator = (function () {
        ...
        tdiv
          .append(SCRGEN_NSCR, scrNum, "&nbsp;", SCRGEN_PRE)
          .append(prefix, "<br>", button, " | ", downclk, "<br>", output);
        var genScrambles = "";
      ...
      $(function () {
        ...
        scrOptDiv.append(
          $("<div>").append(SCRAMBLE_LENGTH + ": ", scrLen),
          scrHelp,
          scrFltDiv,
        );

        title.append(
          $("<nobr>").append(selects[0], " ", selects[1], " ", scrOpt),
          " <wbr>",
        );
        title.append(
          $("<nobr>").append(lastClick, "/", nextClick, SCRAMBLE_SCRAMBLE),
        );
