...
var timer = execMain(
  function (regListener, regProp, getProp, pretty, ui, pushSignal) {
    var container;
    ...
    function reset() {
      ...
      lcd.reset(
        /^[ilvq]$/.exec(type) || (type == "g" && getProp("giiVRC") != "n"),
        type == "i",
      );
    ...
    var lcd = (function () {
        ...
        if (status == -3 || (status == -2 && checkUseIns())) {
          ...
          if (getProp("timeU") != "n") {
            if (time > 17000) {
              ...
              insVal = /^[ab]p$/.exec(getProp("useIns"))
                ? Math.floor(time / 1000)
                : 15 - Math.floor(time / 1000);
            }
          }
      ...
      function setValue(val) {
        setHtml(
          isRight ? rightDiv : mainDiv,
          (val != undefined ? pretty(val, true) : "--:--") +
      ...
      function renderUtil(clear) {
        ...
        var mpAppend =
          status > -2 && !isCleared
            ? getMulPhaseAppend(
                Math.max(0, status),
                Math.max(curTime.length - 1, status),
              )
            : "";
        ...
        if (status == -1 || status == 0) {
          if ("sb".indexOf(getProp("input")) != -1) {
            ...
            if (
              ...
              var label = $(".difflabel").html(
                "(" +
                  (diff > 0 ? "+" : diff == 0 ? "" : "-") +
                  pretty(Math.abs(diff)) +
                  ")",
              );
              ...
              if (diff != 0 && color != "b") {
                label.css(
                  "color",
                  diff > 0 == (color == "gr") ? timerColors[3] : timerColors[4],
                );
              }
            }
          }
          if (value && value[4] && !isCleared) {
            reconsDiv.show();
            var puzzle =
              (typeof value[4][1] == "string" && value[4][1]) ||
              tools.getCurPuzzle() ||
              "333";
            var moveCnt =
              puzzle == "333" ? recons.getMoveCnt(value[4][0]) : value[4][2];
            var txt = STATS_REVIEW;
            if (moveCnt > 0) {
              txt =
                moveCnt +
                " turns<br>" +
                ~~((100000 * moveCnt) / value[0][1]) / 100.0 +
                " tps";
            }
            reconsSpan
              .html(txt)
    ...
    var keyboardTimer = (function () {
      ...
      function onkeydown(keyCode, isTrigger) {
        ...
        } else if (keyCode == 27 && status <= -1) {
          //inspection or ready to start, press ESC to reset
          clearPressReady();
          setStatus(-1);
      ...
      function resetTrain() {
        ...
        div.css(
          "height",
          (getProp("timerSize") * $("#logo").width()) / 12 + "px",
        );
      ...
      function detectTrigger(keyCode, type, e) {
        ...
        return (
          keyCode == 32 || (prevStatus == 3 && keyCode > 255) || ctrlStatus == 3
        );
      }
    ...
    function updateTimerOffset(clear) {
      ...
      if ($("html").hasClass("m") && !clear) {
        ...
        var calcTop = $("#scrambleDiv").is(":visible")
          ? $("#scrambleDiv").outerHeight()
          : 0;
        var calcBottom = $("#stats").offset().top || winHeight;
        if (isToolTop) {
          calcTop += $("#toolsDiv").is(":visible")
            ? $("#toolsDiv").outerHeight()
            : 0;
        } else {
          calcBottom = Math.min(
            calcBottom,
            $("#toolsDiv").offset().top || winHeight,
          );
        }
    ...
    $(function () {
      ...
      regListener(
        ...
        function (signal, value) {
          ...
          if (value[0] == "timerSize" || value[0] == "phases") {
            $("#multiphase").css(
              "font-size",
              getProp("timerSize") / Math.max(getProp("phases"), 4) + "em",
            );
          }
          if (value[0] == "input") {
          ...
          if (
            ["toolPos", "scrHide", "toolHide", "statHide"].indexOf(value[0]) >=
            0
          ) {
          ...
          if (value[0] == "col-timer") {
            timerColors = (value[1] || "#f00#0d0#dd0#080#f00").match(
              /#[0-9a-fA-F]{3}/g,
            );
          }
      ...
      regProp(
        ...
        1,
        PROPERTY_VRCSPEED,
        [100, [0, 50, 100, 200, 500, 1000], "\u221E|20|10|5|2|1".split("|")],
        1,
      );
      regProp(
        "vrc",
      ...
      regProp("vrc", "giiRST", 1, PROPERTY_GIIRST, [
        "p",
        ["a", "p", "n"],
      ...
      regProp(
        ...
        [
          20100,
          [1, 100, 1000, 10001, 10100, 11000, 20001, 20100, 21000],
      ...
      regProp(
        ...
        1,
        PROPERTY_PRETIME,
        [300, [0, 300, 550, 1000], "0|0.3|0.55|1".split("|")],
        1,
      );
      ...
      regProp(
        ...
        1,
        "Stop/step timer by",
        ["a", ["a", "l", "s"], "any key|spacebar/letters|spacebar".split("|")],
        1,
      );
      regProp(
        "timer",
        "phaseBeep",
        0,
        "Beep when recording phase times",
        [true],
        1,
      );
