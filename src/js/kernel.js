...
var kernel = execMain(function () {
  ...
  function pushSignal(signal, value) {
    ...
    if (listeners[signal] != undefined) {
      for (var module in listeners[signal]) {
        ...
        if (
          listeners[signal][module][1] === undefined ||
          listeners[signal][module][1].exec(value[0])
        ) {
  ...
  var property = (function () {
    ...
    table.append(
      $("<tr />").append(left, right.append(scrollDiv.append(optTable))),
    );
    ...
    function scrollToModule(module) {
      setTimeout(function () {
        scrollDiv.scrollTop(
          module
            ? scrollDiv.scrollTop() + subDivs[module][1].position().top - 3
            : prevScrollTop,
        );
      }, 0);
    }
      ...
      if (target.is(".opthelp")) {
        if (target.html() == DEFAULT_HELP_SPAN) {
          target.html(
            "<br>" +
              DEFAULT_HELP_SPAN +
              $('strong[data="opt_' + target.attr("data") + '"]')
                .parent()
                .html()
                .split("</strong>. ")[1],
          );
        ...
        switch (target.prop("type")) {
          ...
          case "button":
            for (var module in proSets)
              if (key in proSets[module]) {
                var obj = proSets[module][key];
                var newVal = getProp(key);
                switch (target.val()) {
                  case "+":
                    newVal = Math.min(newVal + 1, obj[3][2]);
                    break;
                  case "-":
                    newVal = Math.max(newVal - 1, obj[3][1]);
                    break;
                  default:
    ...
    function generateDiv() {
      ...
      for (var module in MODULE_NAMES) {
        ...
        curDiv[1].append(
          $("<th>").html(
            '<span class="icon">' +
              moduleIcon[module] +
              "</span> " +
              MODULE_NAMES[module].replace(/-?<br>-?/g, ""),
          ),
        ...
        for (var key in proSets[module]) {
          ...
          if (type == 0) {
            //checkbox
            proSet[0] = $('<input type="checkbox" name="' + key + '">')
              .prop("checked", curVal)
              .click(procClick);
            ...
            //range
            proSet[0] = $(
              '<input type="text" maxlength="4" name="' + key + '">',
            )
              .val(curVal)
              .change(procClick);
            var inc = $(
            ...
            valTd.append(
              proSet[2] + "(" + proSet[3][1] + "~" + proSet[3][2] + ")",
              $("<span>")
                .css("white-space", "nowrap")
                .append(dec, proSet[0], inc),
            );
          } else if (type == 3) {
            //color
            proSet[0] = $('<input type="color" name="' + key + '">')
              .val(curVal)
              .change(procClick);
            valTd.append(proSet[2], ": ", proSet[0]);
          } else if (type == 4) {
          ...
          if ($('strong[data="opt_' + key + '"]').length > 0) {
            valTd.append(
              $('<span class="click opthelp" data="' + key + '"/>')
                .html(DEFAULT_HELP_SPAN)
                .click(procClick),
            );
          }
    ...
    function showDiv() {
      ...
      ui.showDialog(
        [
          ...
          [BUTTON_EXPORT.replace(/-?<br>-?/g, ""), exportFunc.exportProperties],
        ],
        "option",
    ...
    function setProp(key, value, signalType) {
      for (var module in proSets) {
        if (
          key in proSets[module] &&
          proSets[module][key][0] !== undefined &&
  ...
  $(function () {
    ...
    regProp("kernel", "lang", 1, "Language", [
      curLang,
    ...
    if ($.urlParam("lang")) {
      ...
      document.cookie =
        langParam + "; expires=Fri, 31 Dec 9999 23:59:59 GMT; path=/";
      $.clearUrl("lang");
    }
  });
  ...
  var ui = (function () {
    ...
    function windowClick() {
      ...
      if (modules[module].button) {
        ...
        if (!isHide || !modules[module].auto) {
          modules[module].div.stop(true, true).fadeOut(
            200,
            (function (module) {
              return function () {
                pushSignal("button", [module, false]);
              };
            })(module),
          );
        }
    ...
    function addWindow(module, button, div, enable, auto, index) {
      ...
      div.append(
        $('<span class="chide" data="' + hideMap[module] + '"></span>').click(
          toggleHide,
        ),
      );
    ...
    dialog.append(
      $("<table>").append(
        $('<tr style="height:0%;">').append($("<td>").append(title)),
        $('<tr style="height:100%;">').append(
          $('<td style="position:relative;">').append(value),
        ),
        $('<tr style="height:0%;">').append($("<td>").append(buttons)),
      ),
    );
    ...
    var refreshButton = $(
      '<span style="float:left;" class="click">&nbsp;\u21bb&nbsp;</span>',
    );
    ...
    function showDialog(values, diagclass, titlestr, callback) {
      ...
      if (values[2] != undefined) {
        buttons.append(
          CAN.unbind("click").click(function () {
            $.waitUser.call();
            values[2] && values[2]();
            hideDialog();
          }),
    ...
    csstmp = [csstmp + "{box-shadow:0 0 .6em ?}", csstmp + "{box-shadow:none}"];
    ...
    function releaseColor() {
      var cssval =
        getProp("uidesign") == "ns" || getProp("uidesign") == "mtns"
          ? csstmp[1]
          : csstmp[0];
      var sgn = $.nearColor(cur_color[0]) == "#000" ? -1 : 1;
      for (var i = 0; i < col_map.length; i++) {
        var stdcolor = $.nearColor(
          cur_color[col_map[i] & 0xf],
          ((col_map[i] << 20) >> 24) * sgn,
        );
    ...
    function procSignal(signal, value) {
      if (signal == "property") {
        switch (value[0]) {
          ...
          case "font":
            if (value[1] == "r") {
              $("#container, #multiphase").css(
                "font-family",
                ["lcd", "lcd2", "lcd3", "lcd4", "lcd5"][~~(Math.random() * 5)],
              );
            } else {
          ...
          case "col-pbs":
            setColor(
              col_props.indexOf(value[0].substring(4, value[0].length)),
              value[1],
            );
            break;
          case "zoom":
    ...
    $(function () {
      ...
      regProp(
        ...
        [
          "1",
          ["0.7", "0.8", "0.9", "1", "1.1", "1.25", "1.5"],
          ["70%", "80%", "90%", "100%", "110%", "125%", "150%"],
        ],
        1,
      );
      regProp("ui", "font", 1, PROPERTY_FONT, [
        "lcd",
        ["r", "Arial", "lcd", "lcd2", "lcd3", "lcd4", "lcd5", "Roboto"],
        PROPERTY_FONT_STR.split("|").concat("Roboto"),
      ]);
      regProp("kernel", "ahide", 0, PROPERTY_AHIDE, [true], 1);
      regProp("ui", "uidesign", 1, PROPERTY_UIDESIGN, [
        "n",
        ["n", "mt", "ns", "mtns", "cspt"],
        PROPERTY_UIDESIGN_STR.split("|").concat("csTimer+"),
      ]);
      regProp("ui", "view", 1, PROPERTY_VIEW, [
        "a",
        ["a", "m", "d"],
        PROPERTY_VIEW_STR.split("|"),
      ]);
      regProp("color", "color", 1, PROPERTY_COLOR, [
        "1",
        ["u", "e", "r", "1", "2", "3", "4", "5", "6", "7", "8"],
      ...
      regProp("color", "colmgm", 4, "Megaminx", [
      ...
      regProp("color", "colfto", 4, "FTO", [
        "#fff#808#0d0#f00#00f#bbb#ff0#fa0",
      ]);
      regProp("color", "colico", 4, "ICO", [
        "#fff#084#b36#a85#088#811#e71#b9b#05a#ed1#888#6a3#e8b#a52#6cb#c10#fa0#536#49c#ec9",
      ]);

      regProp("ui", "wndScr", 1, PROPERTY_WNDSCR, [
        "n",
        ["n", "f"],
        PROPERTY_WND_STR.split("|"),
      ]);
      regProp("ui", "wndStat", 1, PROPERTY_WNDSTAT, [
        "n",
        ["n", "f"],
        PROPERTY_WND_STR.split("|"),
      ]);
      regProp("ui", "wndTool", 1, PROPERTY_WNDTOOL, [
        "n",
        ["n", "f"],
        PROPERTY_WND_STR.split("|"),
      ]);
    ...
    _.reset = function (val) {
      ...
      for (var i = 0; i < this.data.length; i++) {
        this.select1.append(
          $("<option>")
            .html(this.data[i][0])
            .val($.isArray(this.data[i][1]) ? i : this.data[i][1])
  ...
  var bgImage = (function () {
    ...
    $(function () {
      ...
      regProp("ui", "bgImgS", 1, BGIMAGE_IMAGE, [
        "n",
        ["n", "u", 0, 1, 2, "f"],
