var ftosolver = (function () {
  ...
  FtoCubie.prototype.fromFacelet = function (facelet) {
    ...
    var co = [];
    if (
      mathlib.detectFacelet(cornFacelets, f, this.cp, co, 9) == -1 ||
  ...
  function genCkmv(moves) {
    ...
    for (var m1 = 0; m1 < moves.length; m1++) {
      ...
      for (var m2 = 0; m2 < m1; m2++) {
        FtoCubie.FtoMult(
          FtoCubie.moveCube[moves[m1]],
          FtoCubie.moveCube[moves[m2]],
          tmp1,
        );
        FtoCubie.FtoMult(
          FtoCubie.moveCube[moves[m2]],
  ...
  function phase1Init() {
    var fc = new FtoCubie();
    p1epMoves = mathlib.createMoveHash(
      fc.ep.slice(),
      phase1Moves,
      phase1EdgeHash,
      ftoPermMove.bind(null, "ep"),
    );
    p1rlMoves = mathlib.createMoveHash(
      fc.rl.slice(),
      phase1Moves,
      phase1CtrlHash,
      ftoPermMove.bind(null, "rl"),
    );
  ...
  function phase1ProcSol(sol, solsym, fc) {
    ...
    for (var i = 0; i < std[0].length; i++) {
      var move = std[0][i];
      sol[i] =
        FtoCubie.symMulM[FtoCubie.symMulI[0][solsym[1]]][move >> 1] * 2 +
        (move & 1);
  ...
  function phase2Init() {
    var fc = new FtoCubie();
    p2epMoves = mathlib.createMoveHash(
      fc.ep.slice(),
      phase2Moves,
      phase2EdgeHash,
      ftoPermMove.bind(null, "ep"),
    );
    p2rlMoves = mathlib.createMoveHash(
      fc.rl.slice(),
      phase2Moves,
      phase2CtHash,
      ftoPermMove.bind(null, "rl"),
    );
    p2ccMoves = mathlib.createMoveHash(
      fc,
      phase2Moves,
      phase2CpcoHash,
      ftoFullMove,
    ...
    var p2necPrun = [
      // idx = (a << 2 | b) * 7 + c, a: #mismatch in U faces, b: U corners w/o U faces, c: others
      0,
      99, 3, 4, 5, 6, 8, 99, 2, 3, 4, 5, 6, 8, 1, 3, 4, 5, 6, 7, 8, 1, 3, 4, 5,
      6, 7, 9, 99, 2, 3, 4, 5, 6, 8, 2, 2, 4, 4, 5, 6, 8, 3, 3, 4, 5, 6, 7, 8,
      3, 3, 4, 5, 6, 7, 9, 3, 3, 4, 5, 6, 7, 8, 4, 4, 4, 5, 6, 7, 8, 4, 4, 5, 6,
      7, 8, 9, 4, 4, 5, 6, 7, 8, 9, 4, 4, 5, 6, 7, 8, 9, 4, 4, 5, 6, 7, 8, 9, 5,
      5, 6, 7, 8, 9, 10, 5, 5, 6, 7, 8, 9, 10,
    ];
    ...
    solv2 = new mathlib.Searcher(
      ...
      function (idx) {
        var xors =
          ufStd2Bit[idx[3] >> 4] ^ cc2Bit[p2ccRecol[idx[3] & 0xf][idx[2]]];
        xors = (xors | (xors >> 1)) & 0x555555;
        var necIdx =
          ((mathlib.bitCount(xors & 0x3f) << 2) |
            mathlib.bitCount(xors & 0xc0c0c0)) *
            7 +
          mathlib.bitCount(xors & 0x3f3f00);
        return Math.max(
          Math.min(
            P2EPRL_MAXL,
            mathlib.getPruning(p2eprlPrun, idx[1] * N_P2EP + idx[0]),
          ),
          p2necPrun[necIdx],
        );
      },
  ...
  function solvePhase2(solvInfos) {
    ...
    for (var i = 0; i < sol.length; i++) {
      ...
      sol[i] =
        FtoCubie.symMulM[FtoCubie.symMulI[0][solvInfo[3]]][move >> 1] * 2 +
        (move & 1);
  ...
  function phase3Init() {
    var fc = new FtoCubie();
    p3epMoves = mathlib.createMoveHash(
      fc.ep.slice(),
      phase3Moves,
      phase3EdgeHash,
      ftoPermMove.bind(null, "ep"),
    );
    p3ufMoves = mathlib.createMoveHash(
      new FtoCubie(),
      phase3Moves,
      phase3CcufHash,
      ftoFullMove,
  ...
  FtoSolver.prototype.solveFto = function (fc, invSol) {
    ...
    solvInfo2[0] = FtoCubie.FtoMult(
      pyraSymCube[FtoCubie.symMulI[0][~~(sym1Idx / 12)]],
      solvInfo2[0],
      null,
    );
  ...
  function solveTest(n_moves) {
    ...
    DEBUG &&
      console.log(
        "after phase 1 (" + prettyMoves(solver.sol1) + "):\n",
        fc.toString(1),
      );
    fc = applyMoves(fc, solver.sol2);
    DEBUG &&
      console.log(
        "after phase 2 (" + prettyMoves(solver.sol2) + "):\n",
        fc.toString(1),
      );
    fc = applyMoves(fc, solver.sol3);
    DEBUG &&
      console.log(
        "after phase 3 (" + prettyMoves(solver.sol3) + "):\n",
        fc.toString(1),
      );
