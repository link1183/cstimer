...
var grouplib = (function (rn) {
  ...
  SubgroupSolver.prototype.midCosetHash = function (perm) {
    return this.sgsM == null
      ? this.sgsG.isMember(permInv(perm), this.sgsMdepth)
  ...
  SubgroupSolver.prototype.initPrunTable = function (perm, tryNISS) {
    ...
    if (tryNISS) {
      for (var i = 0; i < this.clen; i++) {
        prunTable.push(-1, -1);
        permMove[i] =
          this.coset2idx[this.midCosetHash(permMult(perm, this.idx2coset[i]))];
  ...
  SubgroupSolver.prototype.DissectionSolve = function (
    ...
    this.initTables();
    DEBUG &&
      console.log("[Subgroup Solver] finished init tables, tt=", $.now() - tt);
    ...
    var tryNISS =
      (permCtx.mask & SubgroupSolver.ALLOW_PRE) != 0 && this.isCosetSearch;
    var ret = null;
    var prunTable1 = this.prunTable;
    var prunTable2 = null;
    if (tryNISS) {
      permCtx.prunTable =
        permCtx.prunTable || this.initPrunTable(perm, tryNISS);
    ...
    for (
      var depth = Math.max(minl, prunTable1[0][pidx]);
      depth <= maxl;
      depth++
    ) {
      var s1tot = 0;
      ...
      for (var mpidx = 0; mpidx < this.clen * preSize; mpidx++) {
        ...
        var mpidx2 =
          mpidx +
          (tryNISS ? (mpidx >= this.clen ? -this.clen : this.clen * 2) : 0);
        if (
