...
var cubeutil = (function () {
  ...
  var crossMask = toEqus(
    "----U--------R--R-----F--F--D-DDD-D-----L--L-----B--B-",
  );
  var f2l1Mask = toEqus(
  ...
  var ollMask = toEqus(
    "UUUUUUUUU---RRRRRR---FFFFFFDDDDDDDDD---LLLLLL---BBBBBB",
  );
  var eollMask = toEqus(
    "-U-UUU-U----RRRRRR---FFFFFFDDDDDDDDD---LLLLLL---BBBBBB",
  );
  var cpllMask = toEqus(
    "UUUUUUUUUr-rRRRRRRf-fFFFFFFDDDDDDDDDl-lLLLLLLb-bBBBBBB",
  );
  var roux1Mask = toEqus(
    "---------------------F--F--D--D--D-----LLLLLL-----B--B",
  );
  var roux2Mask = toEqus(
    "------------RRRRRR---F-FF-FD-DD-DD-D---LLLLLL---B-BB-B",
  );
  var roux3Mask = toEqus(
    "U-U---U-Ur-rRRRRRRf-fF-FF-FD-DD-DD-Dl-lLLLLLLb-bB-BB-B",
  );
  ...
  function solvedProgressCubie(param, mask) {
    var faceGetter = function (i) {
      ...
      if (!src) {
        ...
        ret =
          mathlib.CubieCube.cFacelet[val & 0x7][(3 - (val >> 3) + src[2]) % 3];
      } else if (src[0] == 1) {
  ...
  var identPLL = (function () {
    ...
    return function (facelet) {
      for (var i = pllPattern.length; i < 22; i++) {
        var param =
          i == 21 ? "UUUUUUUUUFFFRRRBBBLLL" : scramble_333.getPLLImage(i)[0];
        pllPattern.push(
          toEqus(
            LLPattern.replace(/[0-9a-z]/g, function (v) {
  ...
  var identC2CLL = (function () {
    ...
    return function (facelet) {
      for (var i = cllPattern.length; i < 40; i++) {
        ...
        cllPattern.push(
          toEqus(
            c2LLPattern.replace(/[0-9a-z]/g, function (v) {
              return param[parseInt(v, 36)].toLowerCase();
            }),
          ),
        );
  ...
  function getPrettyReconstruction(rawMoves, method) {
    ...
    for (var i = 0; i < prettyMoves.length; i++) {
      ...
      prettySolve +=
        ...
        (stepNames[i]
          ? " // " + stepNames[i] + " " + prettyMoves[i][1] + " move(s)"
          : "") +
        "\n";
    }
    return {
  ...
  function getConjMoves(moves, inv, conj) {
    ...
    return moves.replace(/[URFDLB]/g, function (face) {
      return "URFDLB".charAt(
        mathlib.CubieCube.rotMulM[conj]["URFDLB".indexOf(face) * 3] / 3,
      );
    });
  }

  function getPreConj() {
    // TODO 24 cases, use map insetad of calculation
    var preScr = kernel
