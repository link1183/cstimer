  ...
  function (CubieCube) {
    ...
    function setScramble(scramble) {
      ...
      for (var i = 0; i < scr.length; i++) {
        ...
        scrState.selfMoveStr(
          "URFDLB".charAt(scr[i][0]) + " 2'".charAt(scr[i][2] - 1),
        );
      }
    }
    ...
    function checkState(state) {
      ...
      var toMove = toMoveFix
        ? scrambleToHtml(toMoveFix)
        : scrambleToHtml(toMoveRaw);
    ...
    function scramblePartToHtml(part, cssClass) {
      return (
        part &&
        $.map(part.split(" "), function (move) {
          return (
            move && "<span class='" + cssClass + "'>" + move.trim() + "</span>"
          );
        }).join("")
      );
    }
    ...
    function scrambleToHtml(scrTxt) {
      ...
      var scrParts = $.map(scrTxt.split(":", 3), function (part) {
        return part.trim();
      });
      if (isScrambled) {
...
var giikerutil = execMain(
  function (CubieCube) {
    ...
    var lastSolveClick = $('<a target="_blank">Pretty(N/A)</a>').addClass(
      "click",
    );
    ...
    var canvasTd = $("<tr>").append(
      $('<td colspan=2 style="padding:0;">').append(canvas),
    );
    var deviceName = null;
      ...
      function face(f, facelet) {
        ...
        for (var x = 0; x < 3; x++) {
          for (var y = 0; y < 3; y++) {
            $.ctxDrawPolygon(
              ctx,
              colors["DLBURF".indexOf(facelet[(f * 3 + y) * 3 + x])],
              [
                [x, x, x + 1, x + 1],
                [y, y + 1, y + 1, y],
              ],
    ...
    function renderStatus() {
      ...
      statusDiv.empty();
      statusDiv.append(
        $("<tr>").append($("<td colspan=2>").append(connectClick, debugClick)),
      );
      if (GiikerCube.isConnected() && deviceName) {
        statusDiv
          .append($("<tr>").append(batteryTd, slopeTd))
          .append(
            $("<tr>").append(
              $("<td colspan=2>").append(
                resetClick.unbind("click").click(markSolved),
              ),
            ),
          )
          .append(
            $("<tr>").append(
              $("<td>").append(algCubingClick),
              $("<td>").append(lastSolveClick),
            ),
          )
          .append(canvasTd);
        ...
        connectClick
          .html(TOOLS_GIIKER + "<br>" + GIIKER_CONNECT)
          .unbind("click")
          .click(init);
      }
    ...
    function giikerErrorDetect() {
      ...
      if (kernel.getProp("giiAED")) {
        detectTid = setTimeout(function () {
          ...
          if (cubeutil.getProgress(facelet, "cfop") <= 2) {
            // all unsolved pieces is on same face
            return;
          }
          ...
          if (gen.length / 3 < 10) {
            DEBUG &&
              console.log(
                "[giiker]",
                "Possible error, gen=" + gen.replace(/ /g, "") + ", ignore",
              );
            return;
          }
          DEBUG &&
            console.log(
              "[giiker]",
              "Almost error, gen=" + gen.replace(/ /g, "") + ", mark solved",
            );
          markSolved();
        }, 1000);
      }
    ...
    function checkMoves(moveTsList) {
      ...
      for (var i = moveTsList.length - 1; i >= 0; i--) {
        ...
        CubieCube.CubeMult(
          CubieCube.moveCube[moveTsList[i][0]],
          stateToEnd[i + 1],
          stateToEnd[i],
        );
      }
    ...
    function checkSwap(moveTsList, start, nswap, stateFromStart, stateToEnd) {
      if (nswap == 0) {
        return stateFromStart.isEqual(
          new CubieCube().invFrom(stateToEnd[start]),
        );
      }
      ...
      for (var i = start; i < moveTsList.length - 1; i++) {
        ...
        CubieCube.CubeMult(
          state,
          CubieCube.moveCube[moveTsList[i + 1][0]],
          cctmp,
        );
    ...
    function giikerCallback(facelet, prevMoves, lastTs, hardware) {
      ...
      if (prevMoves.length > 0) {
        var move =
          "URFDLB".indexOf(prevMoves[0][0]) * 3 +
          " 2'".indexOf(prevMoves[0][1]);
    ...
    function tsLinearFit(moveTsList, inv) {
      ...
      var r2 =
        varX < 1e-3 || varY < 1e-3 ? 1 : Math.pow(covXY, 2) / varX / varY;
      var s_slope =
        n < 3 || varX < 1e-3
          ? 0
          : Math.sqrt(((varY / varX) * (1 - r2)) / (n - 2));
    ...
    function tsLinearFix(moveTsList, startTime, endTime) {
      ...
      for (var i = 0; i < moveTsList.length; i++) {
        moveTsList[i][1] =
          Math.round(slope * moveTsList[i][1] + intercept) - startTime;
      }
      return moveTsList;
    }
    ...
    function updateSlopeSpan() {
      if (
        moveTsList.length - moveTsStart > 50 &&
        moveTsStart > 0 &&
        moveTsList[moveTsStart - 1][2] < timer.getStartTime()
      ) {
      ...
      slopeTd.html(
        slopeIcon + Math.round((param[0] - 1) * 100000) / 1000 + "%",
      );
    }
      ...
      if (setup || alg) {
        click.attr(
          ...
          "https://alg.cubing.net/?alg=" +
            encodeURIComponent(alg) +
            "&setup=" +
            encodeURIComponent(setup),
        );
      } else {
    ...
    function setLastSolve(solve) {
      updateAlgClick(
        lastSolveClick,
        "Pretty",
        cubeutil.getConjMoves(curScramble),
        solve,
      );
    }
    ...
    function reconsSolve() {
      ...
      if (
        startIdx > 0 &&
        moveTsList[startIdx - 1][2] > toReconsSolve[0] + toReconsSolve[1] + 1000
      ) {
        toReconsSolve = null;
      }
      for (var i = moveTsList.length - 1; i >= 0; i--) {
    ...
    var debugInfo = (function () {
      ...
      function showDialog() {
        ...
        kernel.showDialog(
          [debugText, closeCallback, closeCallback, closeCallback],
          "share",
          "Bluetooth Debug",
        );
      }
      ...
      function appendLog() {
        ...
        DEBUG &&
          console.log.apply(null, [].concat([ts], Array.from(arguments)));
        msgs.push(msg.join(" "));
        if (msgs.length > 1000) {
          msgs = msgs.slice(100);
        }
