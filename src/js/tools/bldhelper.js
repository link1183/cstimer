...
var bldhelper = execMain(function () {
  ...
  function cntOris(n, base, nDone, nErr) {
    ...
    if (nDone + nErr < n) {
      ...
      return [
        Math.pow(base - 1, nErr) * Math.pow(base, n - nDone - nErr - 1),
        mathlib.valuedArray(nDone, 0).concat(ret),
      ];
    }
  ...
  function getRandState(
    ...
    for (var i = 0; i < 3; i++) {
      ...
      var anySamp = 0;
      for (
        var j = Math.max(nerrLR[0] - fixErr, 0);
        j <= Math.min(nerrLR[1] - fixErr, anyInp);
        j++
      ) {
        var cur = cntOris(
          cycCnt + inpCnt,
      ...
      cntValid += curValid;
      if (!mathlib.rndHit(curValid / cntValid)) {
        // not sampled
        continue;
      }
      ...
      if (i == 2 && perm.indexOf(0) >= oupCnt) {
        var offset =
          (perm.indexOf(0) - mathlib.rn(oupCnt) + remainCnt) % remainCnt;
  ...
  function getBLDcode(c, scheme, cbuf, ebuf, order) {
    ...
    while (done != 0xff) {
      ...
      if (target == cbuf) {
        // buffer in place, swap with any unsolved
        var perm = -1;
  ...
  function genBLDRndState(bldSets, doScramble) {
    ...
    for (var i = 0; i < cfixs.length; i++) {
      ...
      if (!m) {
        continue;
      } else if (m[1] == cubies[bldSets["cbuff"][0] % 8]) {
        // buffer
        continue;
      } else if (m[2]) {
    ...
    for (var i = 0; i < efixs.length; i++) {
      ...
      if (!m) {
        continue;
      } else if (m[1] == cubies[(bldSets["ebuff"][0] % 12) + 8]) {
        // buffer
        continue;
      } else if (m[2]) {
    ...
    var cc = new mathlib.CubieCube();
    var rndX = bldSets["ceori"]
      ? mathlib.rndEl(["", "Rw", "Rw2", "Rw'", "Fw", "Fw'"])
      : "";
    ...
    if (move2 != null) {
      cc.selfMoveStr(
        "URFDLB".charAt(~~(move2 / 3)) + " 2'".charAt(move2 % 3),
        true,
      );
    }
    if (move1 != null) {
      cc.selfMoveStr(
        "URFDLB".charAt(~~(move1 / 3)) + " 2'".charAt(move1 % 3),
        true,
      );
    }
    var facelet = cc.toFaceCube();
    return (
      (scramble_333.genFacelet(facelet) + " " + rndX + " " + rndY).replace(
        / +/g,
        " ",
      ) || "U U'"
    );
  ...
  function procBLDSetEvent(e) {
    ...
    if (/^[ce]buff[01]$/.exec(key)) {
      ...
      if (val == "default") {
        ...
        var ret = prompt(
          "Code order",
          ordStr.slice(0, 8) + " " + ordStr.slice(8),
        );
        if (!ret) {
          updateSchemeSelect();
          return;
  ...
  function genBLDSetTable(bldSets, setDiv) {
    ...
    setDiv.append(
      $("<tr>").append(
        $("<th>Coder</th>"),
        $("<td colspan=2>").append(schSel, ordSel),
      ),
    );
    setDiv.append(
      $("<tr>").append($('<td colspan=3 style="width:0;">').append(codeDiv)),
    );
    setDiv.append(
    ...
    setDiv.append(
      $("<tr>").append(
        "<td>buffer</td>",
        $("<td>").append(cbufSel, cbufFlt),
        $("<td>").append(ebufSel, ebufFlt),
      ),
    );
    setDiv.append(
      $("<tr>").append(
        ...
        $("<td colspan=2>").append(
          (ret[0] == 0
            ...
            : prob < 1e-3
              ...
              : Math.round(prob * 1000000) / 10000 + "%") +
            (prob < 1e-8
              ? "<br>N=" + (ret[0] > 1e8 ? ret[0].toExponential(3) : ret[0])
              : ""),
        ),
      ),
    );
  ...
  function calcResult() {
    ...
    codeDiv.html(
      "C: " + codes[0].join("") + "<br>" + "E: " + codes[1].join(""),
    );
    if (DEBUG) {
      var regen = checkBLDcode(
        [
          codes[0].join("").replace(/\s/g, ""),
          codes[1].join("").replace(/\s/g, ""),
        ],
        bldSets["scheme"],
        bldSets["cbuff"][0],
        bldSets["ebuff"][0],
