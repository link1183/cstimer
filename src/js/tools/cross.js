...
var cross = (function (
  ...
  var solvCross = new mathlib.Searcher(
    (idx) => idx[0] + idx[1] == 0,
    (idx) =>
      Math.max(getPruning(permPrun, idx[0]), getPruning(flipPrun, idx[1])),
  ...
  var solvXXXCross = new mathlib.Searcher(
    ...
    (idx) =>
      Math.max(
        ...
        getPruning(
          xxPrun01,
          idx[3] * 24 + idx[2] + 576 * (idx[5] * 24 + idx[4]),
        ),
        getPruning(
          xxPrun02,
          idx[3] * 24 + idx[2] + 576 * (idx[7] * 24 + idx[6]),
        ),
      ),
    (idx, move) => [
      pmv(idx[0], move),
  ...
  function solve_xxcross(moves, face, is3x) {
    ...
    var tt = +new Date();
    var sol = is3x
      ? solvXXXCross.solveMulti(id3s, 0, 20)
      : solvXXCross.solveMulti(idxs, 0, 20);
      ...
      for (var i = 0; i < sample; i++) {
        ...
        for (var j = 0; j < 4; j++) {
          ...
          var sol = solvXCross.solve(
            [perm, flip, edges[j], corns[j], j],
            0,
            isFound ? minLen - 1 : maxLen,
          );
          if (sol == null) {
            continue;
      ...
      function execCross(scramble, fdiv) {
        ...
        for (var face = 0; face < 6; face++) {
          ...
          span.append(
            ...
            clk,
            "): " + rotIdx[face],
            tools.getSolutionSpan(solutions[face]),
            "<br>",
          );
          fdiv.append(span);
        }
