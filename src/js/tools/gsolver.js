(function () {
  ...
  var rubiksCube = (function () {
    ...
    function toAlgLink(meta, sols, ori) {
      ...
      for (var i = 0; i < sols.length; i++) {
        ...
        solstr +=
          sols[i].join(" ").replace(/\s+/g, " ") +
          " // " +
          meta[i]["head"] +
          (sols[i].length == 0 ? " skip" : "") +
          "\n";
      }
      ...
      for (var i = 0; i < meta.length; i++) {
        if (!meta[i]["solv"]) {
          ...
          for (var solved in meta[i]["step"]) {
            meta[i]["solv"][solved] = new mathlib.gSolver(
              [solved],
              cubeMove,
              meta[i]["move"],
            );
        ...
        span.append(
          meta[i]["head"] + ": ",
          sols[i].length == 0 ? "&nbsp;(skip)" : tools.getSolutionSpan(sols[i]),
          "<br>",
        );
        ...
        DEBUG &&
          console.log(
            ...
            "->",
            ret[1],
            stateInit(cubeMove, mathlib.SOLVED_FACELET),
            +new Date() - t,
          );
      }
      span.append(
        $('<a class="click" target="_blank">alg.cubing.net</a>').attr(
          "href",
          toAlgLink(meta, sols, curOri) +
            "&setup=" +
            encodeURIComponent(curScrambleStr),
        ),
      );
    }
    ...
    function block222Solver(scramble, span) {
      ...
      for (var i = 0; i < curScramble.length; i++) {
        curScramble[i] =
          "URFDLB".charAt(curScramble[i][0]) +
          " 2'".charAt(curScramble[i][2] - 1);
      }
      ...
      block222solv =
        block222solv || new mathlib.gSolver(faceSolved, cubeMove, moves);
      for (var i = 0; i < 8; i++) {
    ...
    function exec333StepSolver(type, scramble, span) {
      ...
      for (var i = 0; i < curScramble.length; i++) {
        curScramble[i] =
          moveMap.charAt(curScramble[i][0]) +
          " 2'".charAt(curScramble[i][2] - 1);
      }
      span.append(
        "Orientation:",
        oriSelect.unbind("change").change(oriChange),
        "<br>",
      );
      if (type == "cf") {
  ...
  var sq1Cube = (function () {
    ...
    function sq1Solver(scramble, span) {
      ...
      sol = [];
      var sol1 = solv1.search(
        stateInit(sq1Move, "0Aa0Aa0Aa0Aa|Aa0Aa0Aa0Aa0"),
        0,
      );
      span.append("Shape: ", tools.getSolutionSpan(prettySq1Arr(sol1)), "<br>");
      sol = sol.concat(sol1);
      var sol2 = solv2.search(
        stateInit(sq1Move, "0Aa0Aa0Aa0Aa|Bb1Bb1Bb1Bb1"),
        0,
      );
        ...
        out: for (var depth = 0; depth < 99; depth++) {
          for (var j = 0; j < 3; j++) {
            ...
            for (var m = 0; m < scramble.length; m++) {
              curScramble.push(
                rawMap[moveMap.indexOf(scramble[m][0])] + scramble[m][1],
              );
            }
            sol1 = solv.search(
              stateInit(pyraMove, "????FF??RRR??L?L?L?DDDDD"),
              depth,
              depth,
            );
            if (!sol1) {
  ...
  var general333Solver = execMain(function () {
    ...
    var solveButton = $(
      '<input type="button" value="Solve!" style="font-size:0.75em;">',
    );
    var resultText = $(
    ...
    function drawCube(ctx, state) {
      ...
      for (var i = 0; i < 5; i++) {
        for (var j = 0; j < 2; j++) {
          $.ctxDrawPolygon(
            ctx,
            colors["URFDLB-XYZ".charAt(i * 2 + j)],
            [
              [i, i, i + 1, i + 1],
              [j, j + 1, j + 1, j],
            ],
    ...
    function startSolve() {
      if (curSolver[0] != solvedState) {
        ...
        curSolver[1] = new mathlib.gSolver(
          [solvedState],
          rubiksCube.move,
          appendSuffix({
            U: 0x00,
    ...
    function execFunc(scramble, span) {
      ...
      span.empty().append(
        $('<table class="table">').append(
          ...
          $("<tr>").append($("<td>").append(resultText.empty(), resultSpan)),
          $("<tr>").append(
            $('<td style="padding:0;">').append(
              canvas.unbind("mousedown").bind("mousedown", procClick),
              subsetInfo.unbind("click").click(function () {
                clearSolve();
                canvas.show();
                subsetInfo.removeClass("click");
              }),
  ...
  var twophase = execMain(function () {
    ...
    var resultText = $(
      '<textarea rows="8" cols="30" style="font-size:0.75em;">',
    );
    var resultArr = [];
    var canvas, ctx;
    var solvedState = mathlib.SOLVED_FACELET;
      ...
      span
        ...
        .append(
          $('<table class="table">').append(
            ...
            $("<tr>").append($("<td>").append(resultText.empty())),
          ),
        );
  ...
  execMain(function () {
    ...
    $(function () {
      tools.regTool(
        ...
        execFunc.bind(null, "222face"),
      );
      tools.regTool(
        "333cf",
        TOOLS_SOLVERS + ">Cross + F2L",
        execFunc.bind(null, "333cf"),
      );
      tools.regTool(
        "333roux",
        TOOLS_SOLVERS + ">Roux S1 + S2",
        execFunc.bind(null, "333roux"),
      );
      tools.regTool(
        "333petrus",
        TOOLS_SOLVERS + ">2x2x2 + 2x2x3",
        execFunc.bind(null, "333petrus"),
      );
      tools.regTool(
        "333zz",
        TOOLS_SOLVERS + ">EOLine + ZZF2L",
        execFunc.bind(null, "333zz"),
      );
      tools.regTool(
        "333222",
        TOOLS_SOLVERS + ">2x2x2",
        execFunc.bind(null, "333222"),
      );
      tools.regTool(
        "333eodr",
        TOOLS_SOLVERS + ">EO + DR",
        execFunc.bind(null, "333eodr"),
      );
      tools.regTool(
        "sq1cs",
        TOOLS_SOLVERS + ">SQ1 S1 + S2",
        execFunc.bind(null, "sq1cs"),
      );
      tools.regTool(
        "pyrv",
        TOOLS_SOLVERS + ">Pyraminx V",
        execFunc.bind(null, "pyrv"),
      );
      tools.regTool(
        "skbl1",
        TOOLS_SOLVERS + ">Skewb Face",
        execFunc.bind(null, "skbl1"),
      );
      tools.regTool(
        "333udf",
        TOOLS_SOLVERS + ">3x3x3 General",
        execFunc.bind(null, "333udf"),
      );
      tools.regTool(
        "333koc",
        TOOLS_SOLVERS + ">3x3x3 TwoPhase",
        execFunc.bind(null, "333koc"),
      );
    });
  });
})();
