...
var onlinecomp = execMain(function () {
  ...
  var refreshButton = $('<input type="button">')
    .val(OLCOMP_UPDATELIST)
    .click(updateCompList);
    ...
    fdiv
      ...
      .append(
        $('<div style="font-size: 0.75em; text-align: center;">')
          ...
          .append(
            $("<label>").append(anonymInput, OLCOMP_WITHANONYM),
            " ",
            viewMyResultButton,
          ),
      );
    ...
    if (pathSelect.length < 2) {
      ...
      if (compTypes.length == 0) {
        ...
        for (var i = 0; i < compTypes.length; i++) {
          ...
          compProgressDiv.append(
            i +
              1 +
              ". " +
              (solves[i] ? stats.pretty(solves[i][0]) : compTypes[i]),
            "<br>",
          );
        }
  ...
  function viewResult() {
    ...
    $.post(
      ...
      function (value) {
        ...
        var myid = $.sha256(
          "cstimer_public_salt_" + exportFunc.getDataId("locData", "compid"),
        );
        var mywcaid = (exportFunc.getDataId("wcaData", "wca_me") || {})[
          "wca_id"
        ];
        ...
        value = $.map(value["data"], function (val) {
          ...
          var timestat = new TimeStat([5], solves.length, function (idx) {
            return solves[idx][0][0] == -1
              ? -1
              : solves[idx][0][0] + solves[idx][0][1];
          });
        ...
        for (var i = 0; i < value.length; i++) {
          ...
          ret.push(
            kernel.pretty(ao5) + "</td><td>" + kernel.pretty(bo5) + "</td><td>",
          );
          for (var j = 0; j < solves.length; j++) {
            if (solves[j].length > 4) {
              // from vrc or bluetooth
              solves[j][1] = scramble.scrStd("", curScrambles[j] || "")[1];
              ret.push(
                '<a target="_blank" class="click" href="' +
                  stats.getReviewUrl(solves[j]) +
                  '">' +
                  stats.pretty(solves[j][0]) +
                  "</a> ",
              );
            } else {
    ...
    $.post(
      ...
      function (value) {
        ...
        for (var i = 0; i < value.length; i++) {
          ...
          ret.push("<tr><td>" + (i + 1) + "</td>");
          ret.push(
            "<td>" + value[i]["fullname"] + "|" + value[i]["path"] + "</td>",
          );
          ...
          for (var j = 0; j < solves.length; j++) {
            if (solves[j].length > 4) {
              // from vrc or bluetooth
              solves[j][1] = scramble.scrStd(
                "",
                JSON.parse(value[i]["scramble"])[j] || "",
              )[1];
              ret.push(
                '<a target="_blank" class="click" href="' +
                  stats.getReviewUrl(solves[j]) +
