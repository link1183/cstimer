...
var tools = (function () {
  ...
  function scrambleType(scramble) {
    if (
      scramble.match(
        /^([\d]?[xyzFRUBLDfrubldSME]([w]|&sup[\d];)?[2']?\s*)+$/,
      ) == null
    ) {
      return "-";
  ...
  function puzzleType(scrambleType) {
    if (/^222(so|[236o]|eg[012]?|tc[np]?|lsall|nb)$/.exec(scrambleType)) {
      ...
      /^(mgmp|mgmo|mgmc|minx2g|mlsll|mgmpll|mgmll)$/.exec(scrambleType)
    ) {
      return "mgm";
  ...
  var ret = ISCSTIMER
    ? execMain(function () {
        ...
        for (var i = 0; i < 4; i++) {
          fdivs[i] = $("<div />");
          funcSpan[i] = $("<span />");
          funcMenu[i] = new kernel.TwoLvMenu(
            funcData,
            onFuncSelected.bind(null, i),
            $("<select />"),
            $("<select />"),
          );
        ...
        function procSignal(signal, value) {
          if (signal == "property") {
            ...
            kernel.setProp(
              "isTrainScr",
              !!trainScrambleRe.exec((curScramble || [])[0]),
            );
            execFunc(-1, signal);
        ...
        $(function () {
          ...
          var defaultFunc = JSON.stringify([
            "image",
            "stats",
            "cross",
            "distribution",
        ...
        function getSolutionSpan(solution) {
          ...
          for (var i = 0; i < solution.length; i++) {
            span.append(
              '<span style="display:none;">&nbsp;' + solution[i] + "</span>",
            );
          }
          if (kernel.getProp("solSpl")) {
            span.append(
              $('<span class="click" data="n">[+1]</span>').click(
                procSolutionClick,
              ),
            );
            span.append(
              $(
                '<span class="click" data="a">[' +
                  solution.length +
                  "f]</span>",
              ).click(procSolutionClick),
            );
          } else {
            span.children().show();
          }
