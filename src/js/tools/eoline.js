  ...
  function (createMove, edgeMove, createPrun, getPruning) {
    ...
    function solveEOLine(scramble, isCross, fdiv) {
      ...
      for (var face = 0; face < 12; face++) {
        ...
        var sol = isCross
          ? solv2.search([flip, perm1, perm2], 0)
          : solv.search([flip, perm1], 0);
        sol = sol.map(
          (move) => "FRUBLD".charAt(move[0]) + " 2'".charAt(move[1]),
        );
        fdiv.append(
          $('<span class="sol">').append(
            faceStr[face] + ": " + rotIdx[face],
    ...
    $(function () {
      tools.regTool(
        ...
        execFunc.bind(null, "eoline"),
      );
      tools.regTool(
        "eocross",
        TOOLS_SOLVERS + ">" + "EOCross",
        execFunc.bind(null, "eocross"),
      );
    });
  },
  [
