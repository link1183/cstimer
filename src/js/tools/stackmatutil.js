...
var stackmatutil = execMain(function (CubieCube) {
  ...
  function doDebugSample() {
    ...
    stackmat.getSample(0.2, function (data) {
      ...
      for (var i = 1; i < raw.length; i++) {
        ctx.lineTo(
          (i * width) / (raw.length - 1),
          height * 0.5 - height * 0.3 * raw[i],
        );
      }
      ctx.stroke();
      ...
      for (var i = 1; i < bin.length; i++) {
        ctx.lineTo(
          (i * width) / (bin.length - 1),
          height * 0.8 - height * 0.6 * bin[i],
        );
      }
      ctx.stroke();
    });
  ...
  function showDebugDialog() {
    if (!debugDiv) {
      debugDiv = $("<div>");
      debugCanvas = $(
        '<canvas style="display:block; width:95%; margin:auto;">',
      );
    ...
    kernel.showDialog(
      [debugDiv, clearDebug, clearDebug, clearDebug],
      "share",
      "Stackmat Debug",
      doDebugSample,
    );
  ...
  function updateDevices() {
    stackmat.updateInputDevices().then(function (devices) {
      ...
      for (var i = 0; i < devices.length; i++) {
        deviceSelect.append(
          $("<option>").val(devices[i][0]).text(devices[i][1]),
        );
      }
