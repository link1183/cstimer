...
var battle = execMain(function () {
  ...
  function submitSolve(time, isLast) {
    ...
    if (
      localLastSolve &&
      localLastSolve[1] == time[1] &&
      (!isLast || time[0][1] != localLastSolve[0][1])
    ) {
      return;
    }
    localLastSolve = time;
    var solveId =
      solvScr == roomInfo["cur"][1] ? roomInfo["cur"][0] : roomInfo["last"][0];
    conn
      .remoteCall({
        action: "uploadSolve",
        roomId: roomId,
  ...
  function renderRoom() {
    ...
    roomTable.append(
      "<tr><td colspan=2>" + titles.join("</td><td>") + "</td></tr>",
    );
    joinRoomSpan.unbind("click");
    leaveRoomSpan.unbind("click");
    if (!roomInfo) {
      roomTable.append(
        '<tr><td colspan=5 style="width:0;">' +
          TOOLS_BATTLE_INFO +
          "</td></tr>",
      );
      joinRoomSpan
      ...
      for (var i = 0; i < solves.length; i++) {
        ...
        solveDict[accountId][solveObj["solveId"]] = [
          solveObj["time"],
          solveObj["soltime"],
        ];
      ...
      for (var i = 0; i < players.length; i++) {
        ...
        if (account.indexOf("|") != -1) {
          ...
          account =
            account.slice(0, 4) + "..." + account.slice(account.length - 3);
        }
  ...
  function execFunc(fdiv, e) {
    ...
    fdiv
      .empty()
      .append(
        $('<div style="font-size: 0.75em; text-align: center;">').append(
          accountDiv,
          roomTable,
        ),
      );
