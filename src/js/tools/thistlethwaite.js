...
var thistlethwaite = (function () {
  ...
  function initSolvers() {
    ...
    for (var i = 0; i < stepMoves.length; i++) {
      solvs[i] = new grouplib.SubgroupSolver(
        gens[i],
        gens[i + 1],
        middleGroup[i] && move2gen(middleGroup[i]),
      );
      solvs[i].initTables();
    }
    var cc = new mathlib.CubieCube();
  ...
  function enumStepSolutions(step, scramble, useNiss, tryMultiAxis, callback) {
    ...
    for (var depth = 0; depth < 20; depth++) {
      for (var stateAxis = 0; stateAxis < states.length; stateAxis++) {
        ...
        var ret = solvs[step].DissectionSolve(
          states[stateAxis],
          depth,
          depth,
          stateCtx[stateAxis],
          (sol) => {
            ...
            if (sol.length > 0 && allowedEnd != null) {
              ...
              if (
                (start != -1 &&
                  allowedEnd.indexOf(start[0] * 4 + start[1]) == -1) ||
                (end != -1 && allowedEnd.indexOf(end[0] * 4 + end[1]) == -1)
              ) {
                return;
              }
            }
  ...
  function fillStepsCandidates(
    ...
    for (var step = 0; step < 4; step++) {
      ...
      enumStepSolutions(
        ...
        function (sol, prefix) {
          sol =
            (prefix ? prefix + ": " : "") + sol.join(" ").replace(/\s+/g, " ");
  ...
  execMain(function () {
    ...
    function renderTable(step) {
      table.empty();
      nisschk = $('<input type="checkbox" data="niss">').prop(
        "checked",
        useNiss,
      );
      table.append(
        $("<tr>").append(
          ...
          $("<th>").append($("<label>").append("NISS", nisschk)),
        ),
      );
      for (var i = 0; i < 4; i++) {
        ...
        solTd[i].append(
          $(
            '<span class="click" data="step' +
              i +
              '">' +
              toPrettyStyle(skeleton[i]) +
              "</span>",
          ).click(procClick),
        );
      }
