...
var help = execMain(
  function (regProp, setProp, getProp) {
    var docs = {};
    ...
    function updateLeft(val) {
      ...
      if (elems.length == 0) {
        for (var type in docs) {
          $("<div />")
            .html(type)
            .addClass(type == val ? "tab enable" : "tab disable")
            .click(leftClick)
            .appendTo(left);
        }
        elems = left.children();
    ...
    function updateRight(val) {
      setTimeout(function () {
        if (docs[val]) {
          rightDiv.scrollTop(
            rightDiv.scrollTop() + docs[val].position().top - 3,
          );
        }
      }, 0);
      return true;
    ...
    function genColorPreview(colScheme) {
      ...
      obj
        .find(".clpr-bt1")
        .css({ width: "11%", "background-color": colors[3] })
        .html("O");
      obj.find(".clpr-bd").css({ "background-color": colors[2] });
      obj.find(".clpr-lk").css({ color: colors[4] });
      obj
        .find(".clpr-lg")
        .css({ color: colors[5], "background-color": colors[6] });
      obj.click(
        function (url) {
    ...
    function genKeymapTable(layout) {
      ...
      table.append(
        selectTr.append($('<th colspan="10">').append("Layout: ", select)),
      );
