...
execMain(
  function (timer) {
    ...
    function onQiyiTimerEvent(timerEvent) {
      ...
      switch (timerEvent.state) {
        ...
        case CONST.IDLE: // timer reset button pressed
          inspectionTime = 0;
          if (timer.hardTime() > 0 || timer.status() != -1) {
            // reset timer / cancel inspection timer
            timer.hardTime(0);
            timer.status(-1);
            timer.lcd.reset();
            timer.lcd.fixDisplay(false, true);
            ...
            timer.status() == -1 &&
            timer.checkUseIns() &&
            timerEvent.state == CONST.GAN_RESET
          ) {
            // start inspection timer if was idle and inspection enabled in settings
            timer.status(-3);
            timer.startTime($.now());
            timer.lcd.fixDisplay(false, true);
          }
        ...
        case CONST.RUNNING: // timer is started
          if (timer.status() == -3) {
            // if inspection timer was running, record elapsed inspection time
            inspectionTime = $.now() - timer.startTime();
            // 0 == Normal, 2000 == +2, -1 == DNF
            inspectionTime = timer.checkUseIns()
              ? inspectionTime > 17000
                ? -1
    ...
    function reconnectTimer() {
      $.delayExec(
        "bluetoothTimerReconnect",
        function () {
          DEBUG &&
            console.log("[qiyitimer] attempting to reconnect timer device");
          connectTimer(true);
        },
        2500,
      );
    ...
    function connectTimer(reconnect) {
      ...
      BluetoothTimer.init()
        .then(function () {
          DEBUG &&
            console.log("[qiyitimer] timer device successfully connected");
          timer.hardTime(0);
          timer.status(-1);
          timer.lcd.reset();
          timer.lcd.renderUtil();
    ...
    function onKeyUp(keyCode) {
      ...
      if (
        ...
        timer.status() == -4 &&
        timer.checkUseIns() &&
        BluetoothTimer.isConnected() &&
        (timer.hardTime() == 0 || timer.hardTime() == null)
      ) {
        timer.status(-3);
        timer.lcd.reset();
        timer.startTime(now);
