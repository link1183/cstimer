  ...
  function (timer) {
    ...
    function moveListener(move, mstep, ts) {
      ...
      if (timer.status() == -3 || timer.status() == -2) {
        if (
          puzzleObj.isRotation(move) &&
          !/^(333ni|444bld|555bld)$/.exec(curScrType)
        ) {
          if (mstep == 0) {
          ...
          timer.updateMulPhase(
            totPhases,
            puzzleObj.isSolved(kernel.getProp("vrcMP", "n")),
            now,
          );
          fixRelayCounter();
      ...
      if (timer.status() >= 1) {
        if (
          ...
          puzzleObj.toggleColorVisible(
            puzzleObj.isSolved(kernel.getProp("vrcMP", "n")) == 0,
          );
        }
        if (mstep == 0) {
          rawMoves[timer.status() - 1].push([
            puzzleObj.move2str(move),
            now - timer.startTime(),
          ]);
        }
        var curProgress;
        ...
        if (mstep == 2 && curProgress == 0) {
          ...
          kernel.pushSignal("time", [
            ...
            0,
            [
              $.map(rawMoves, cubeutil.moveSeq2str).filter($.trim).join(" "),
              curPuzzle,
              moveCnt,
            ],
          ]);
    ...
    function fixRelayCounter() {
      if (/^r\d+$/.exec(curScrType)) {
        timer.lcd.setStaticAppend(
          curScramble.length + 1 + "/" + relayScrs.length,
        );
        timer.lcd.renderUtil();
      }
    }
    ...
    function procSignal(signal, value) {
      if (signal == "scramble" || signal == "scrambleX") {
        ...
        if (
          (size != -1 || puzzleFactory.twistyre.exec(puzzle)) &&
          (curScrSize != size || curPuzzle != puzzle)
        ) {
          curScrSize = size;
          curPuzzle = puzzle;
          isReseted = false;
