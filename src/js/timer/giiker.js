  ...
  function (timer) {
    ...
    var giikerVRC = (function () {
      ...
      function setState(state, prevMoves, isFast) {
        ...
        if (shouldReset) {
          //cannot get current state according to prevMoves
          resetVRC(false);
          curVRCCubie.fromFacelet(mathlib.SOLVED_FACELET);
        ...
        if (todoMoves.match(/^\s*$/) || !puzzleObj) {
          ...
          scramble = puzzleObj.parseScramble(
            cubeutil.getConjMoves(todoMoves, true, curVRCCubie.ori),
          );
        }
    ...
    function giikerCallback(facelet, prevMoves, lastTs) {
      ...
      if (timer.status() >= 1) {
        ...
        if (isGiiSolved(currentFacelet)) {
          rawMoves.reverse();
          var pretty = cubeutil.getPrettyReconstruction(
            rawMoves,
            solvingMethod,
          );
          ...
          if (timer.curTime()[1] != 0) {
            ...
            for (var i = 0; i < rawMoves.length; i++) {
              ...
              timer.curTime()[rawMoves.length - i] =
                cnt == 0 ? 0 : sol[cnt - 1][1];
            }
    ...
    function canStart(facelet) {
      return (
        facelet != mathlib.SOLVED_FACELET || kernel.getProp("giiMode") != "n"
      );
    }
        ...
        if (kernel.getProp("giiMode") == "n") {
          if (!giikerutil.checkScramble()) {
            ...
            kernel.pushSignal("scramble", [
              "333",
              cubeutil.getConjMoves(gen, true),
              0,
            ]);
          }
